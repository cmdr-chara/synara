...
impl RecapState {
    pub fn open(&self) -> bool { self.dialog.is_some() }
}
impl Shell {
    pub(super) fn open_thread_recap(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let Some(task) = self.task().cloned() else { return; };
        if self.recaps.open() || self.workflows.open() || self.file_comments.open()
            || self.revisions.open() || self.handoff.open() || self.loading_task.is_some()
            || self.close != CloseState::Open || self.explorer.modal_open()
            || self.saved_context.dialog.is_some() || self.organization.dialog.is_some()
            || self.kanban.dialog.is_some()
        { return; }
        let query = cx.new(|cx| TextEntry::new("Filter configured providers and models...", EntryMode::SingleLine, 36., cx));
        ...
        self.recaps.dialog = Some(RecapDialog {
            task: task.id, title: task.title, review: None, selected_model: None,
            query: query.clone(), _subscription: subscription, show_source: false,
            busy: false, cancel: None, error: None, notice: None,
        });
    ...
    fn reload_thread_recap(&mut self, cx: &mut Context<Self>) {
        let Some(d) = self.recaps.dialog.as_mut() else { return; };
        if d.busy { return; }
        d.busy = true;
        d.error = None;
        ...
        self.job(async move { Ok(Update::Recap(Box::new(Reply::Loaded(
            id, epoch, workspace.review_thread_recap(id).await.map_err(|e| e.to_string()),
        )))) });
        cx.notify();
    }
    fn choose_recap_model(&mut self, provider: String, model: String, cx: &mut Context<Self>) {
        if let Some(d) = self.recaps.dialog.as_mut() {
            if !d.busy { d.selected_model = Some((provider, model)); }
        }
        cx.notify();
    }
    fn generate_recap(&mut self, cx: &mut Context<Self>) {
        let Some(d) = self.recaps.dialog.as_mut() else { return; };
        if d.busy || self.selected != Some(d.task) || self.close != CloseState::Open
            || d.query.read(cx).is_composing() { return; }
        let (Some(review), Some((provider, model))) = (d.review.clone(), d.selected_model.clone()) else { return; };
        ...
        self.job(async move { Ok(Update::Recap(Box::new(Reply::Generated(
            id, epoch, controller.generate_thread_recap(review, provider, model, cancel).await.map_err(|e| e.to_string()),
        )))) });
        cx.notify();
    }
    ...
    pub(super) fn recap_reply(&mut self, reply: Reply, cx: &mut Context<Self>) {
        let (id, epoch) = match &reply { Reply::Loaded(id, epoch, _) | Reply::Generated(id, epoch, _) => (*id, *epoch) };
        if epoch != self.recaps.generation || self.selected != Some(id) || self.close != CloseState::Open { return; }
        let Some(d) = self.recaps.dialog.as_mut().filter(|d| d.task == id && d.busy) else { return; };
        d.busy = false;
        d.cancel = None;
        match reply {
            Reply::Loaded(_, _, Ok(review)) => { d.review = Some(review); d.notice = None; }
            Reply::Generated(_, _, Ok(saved)) => {
                if let Some(review) = &mut d.review { review.cached = Some(saved); }
                d.notice = Some("Recap cached separately. The source transcript, session, files and normal draft were not changed.".into());
            }
            Reply::Loaded(_, _, Err(error)) | Reply::Generated(_, _, Err(error)) => { d.error = Some(error); d.notice = None; }
        }
        cx.notify();
    }
    fn dismiss_recap(&mut self, cx: &mut Context<Self>) {
        let Some(d) = &self.recaps.dialog else { return; };
    ...
    pub(super) fn restore_recap_focus(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if self.recaps.restore_focus {
            ...
            if let Some(focus) = self.recaps.previous_focus.take() { window.focus(&focus, cx); }
        }
    }
    pub(super) fn recap_overlay(&self, cx: &mut Context<Self>) -> gpui::AnyElement {
        let Some(d) = &self.recaps.dialog else { return div().into_any_element(); };
        let mut content = div().id("recap-body").min_h_0().flex_1().overflow_y_scroll().flex().flex_col().gap_2();
        if let Some(review) = &d.review {
            ...
            if d.show_source {
                content = content.child(div().id("recap-source-text").h(px(160.)).min_h(px(160.)).flex_shrink_0().overflow_y_scroll()
                    .border_1().border_color(rgb(palette().border)).p_2().text_xs().child(review.snapshot.text.clone()));
            }
            content = content.child(div().relative().h(px(38.)).min_h(px(38.)).flex_shrink_0().child(d.query.clone()).child(ui::layout_probe("recap-model-filter")));
            let query = d.query.read(cx).text().trim().to_lowercase();
            let mut shown = 0;
            let mut matched = 0;
            let mut models = div().id("recap-model-list").max_h(px(140.)).overflow_y_scroll().flex().flex_col().gap_1();
            for p in &review.settings.providers {
                for m in &p.models {
                    let label = format!("{} / {}", p.name, m.name);
                    if !format!("{label} {} {}", p.id, m.id).to_lowercase().contains(&query) { continue; }
                    matched += 1;
                    if shown >= 40 { continue; }
                    let provider = p.id.clone();
                    let model = m.id.clone();
                    let selected = d.selected_model.as_ref().is_some_and(|(a,b)| a == &p.id && b == &m.id);
                    models = models.child(ui::action(format!("recap-model-{shown}"), label, None, selected,
                        cx.listener(move |this, _: &(), _, cx| this.choose_recap_model(provider.clone(), model.clone(), cx)))
                        .relative().child(ui::layout_probe("recap-model")));
                    shown += 1;
                }
            }
            ...
            if !d.busy && d.selected_model.is_some() && source.included_messages > 0 {
                content = content.child(ui::action("recap-generate", if review.cached.is_some() { "Regenerate and replace cache" } else { "Generate and cache recap" },
                    Some(Glyph::Brain), false, cx.listener(|this, _: &(), _, cx| this.generate_recap(cx)))
                    .relative().child(ui::layout_probe("recap-generate")));
            ...
            if let Some(saved) = &review.cached {
                ...
                content = content.child(ui::action("recap-copy", "Copy cached recap", Some(Glyph::Copy), false,
                    cx.listener(move |_, _: &(), _, cx| cx.write_to_clipboard(gpui::ClipboardItem::new_string(text.clone()))))
                    .relative().child(ui::layout_probe("recap-copy")));
            }
        }
        let page = div().id("recap-dialog").role(gpui::Role::Dialog).aria_label("Thread recap").tab_group()
            .w_full().max_w(px(780.)).h(px(740.)).max_h_full().p_4().flex().flex_col().gap_2()
            .bg(ui::surface(palette().overlay)).border_1().border_color(rgb(palette().border))
            .on_mouse_down(gpui::MouseButton::Left, |_, _, cx| cx.stop_propagation())
            .on_key_down(cx.listener(|this, event: &gpui::KeyDownEvent, _, cx| {
                if event.keystroke.key == "escape" && !event.prefer_character_input { this.dismiss_recap(cx); cx.stop_propagation(); }
            }))
            .child(div().text_size(px(20.)).child(format!("Thread recap: {}", d.title)))
            .child(content)
            .children(d.error.as_ref().map(|error| div().relative().text_xs().text_color(rgb(palette().error)).child(error.clone()).child(ui::layout_probe("recap-error"))))
            .children(d.notice.as_ref().map(|notice| div().text_xs().child(notice.clone())))
            .child(div().flex().gap_2().border_t_1().border_color(rgb(palette().border)).pt_2()
                .child(ui::action("recap-close", "Close", None, false, cx.listener(|this, _: &(), _, cx| this.dismiss_recap(cx)))
                    .relative().child(ui::layout_probe("recap-close")))
                .child(ui::action("recap-reload", if d.busy { "Working..." } else { "Reload source and cache" }, None, false,
                    cx.listener(|this, _: &(), _, cx| this.reload_thread_recap(cx)))
                    .relative().child(ui::layout_probe("recap-reload")))
                .children(d.cancel.as_ref().map(|_| ui::action("recap-stop", "Stop recap", Some(Glyph::Stop), false,
                    cx.listener(|this, _: &(), _, cx| this.stop_recap(cx)))
                    .relative().child(ui::layout_probe("recap-stop")))));
        div().id("recap-backdrop").absolute().inset_0().occlude().bg(gpui::rgba(0x00000088))
            .flex().items_center().justify_center().p_3().child(page).into_any_element()
    }
}
