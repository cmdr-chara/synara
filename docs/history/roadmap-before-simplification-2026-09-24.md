...
### Delivered feature slices in the September 24 continuation

The first six continuation batches add **15 bounded product slices** to the 25
previously shipped additions, for **40 slices under this grouping**. The 21 broad
gates still remain OPEN. Exact scope and validation are recorded separately in
the [first batch receipt](docs/verification/parity-2026-09-24-batch1.md) and the
[editor/Studio batch receipt](docs/verification/parity-2026-09-24-batch2.md), and
[setup/worktree/control receipt](docs/verification/parity-2026-09-24-batch3.md), and
[document/export/import receipt](docs/verification/parity-2026-09-24-batch4.md), and
[committed-file history receipt](docs/verification/parity-2026-09-24-batch5.md), and
[web-interaction/search receipt](docs/verification/parity-2026-09-24-batch6.md).

| Batch | Delivered functionality | Count |
| --- | --- | ---: |
| September 24, batch 1 | Explicit authenticated local web task execution with Run/Stop and live transcript/status refresh | 1 |
| September 24, batch 1 | Qualified goal resume, clear and edit commands using the existing goal owner, with explicit Save/Send boundaries | 1 |
| September 24, batch 1 | Reviewed local Simulator app-bundle installation and explicit app termination through the owned native helper | 1 |
| September 24, batch 2 | Per-buffer opt-in idle auto-save through the conflict-checked local writer, preserving later edits and stopping on failure | 1 |
| September 24, batch 2 | Bounded still-WebP decoding for read-only Studio previews, filtering and zoom without rewriting the source | 1 |
| September 24, batch 2 | Durable reporting-turn attribution and a reporting-turn output filter in Studio, explicitly distinct from historical file versions | 1 |
| September 24, batch 3 | Inert setup-chat preparation, explicit ACP Connect/sign-in and connection questions inside onboarding | 1 |
| September 24, batch 3 | Reviewed new local worktree forks with a generated branch, pinned HEAD, preserved source edits and durable unsent task ownership | 1 |
| September 24, batch 3 | Composer-scoped Alt+[ / Alt+] model cycling and exact native model next/previous commands | 1 |
| September 24, batch 4 | Bounded local Hub Library PDF page viewing with immutable snapshots, fit/zoom, reload and cancellation | 1 |
| September 24, batch 4 | Reviewed original Library file export with source recheck and no-overwrite destination publication | 1 |
| September 24, batch 4 | Inline onboarding history discovery, preview and import using the existing owner, with pending-operation navigation guards | 1 |
| September 24, batch 5 | Commit-pinned exact-path file history, read-only revision preview and copy with unsaved buffer/Undo preservation | 1 |
| September 24, batch 6 | Task-scoped web one-time permission and structured question reviews through the existing interaction broker, with expiring receipts and draft-preserving polling | 1 |
| September 24, batch 6 | Debounced/coalesced Explorer name/content search with exact workspace ownership and usable narrow-pane result hit targets | 1 |
...
| Headless / web workspace | Loopback-only headless server with recovery, process ownership and readiness; authenticated project registration, draft editing and explicit local ACP Run/Stop, bounded status polling and one-time task permission/question reviews | Connection sign-in, richer approval context and durable question drafts in the web workspace, direct-model and remote execution, remote bind/TLS/deployment/update policy and release packaging |
| First-run onboarding / setup replay | Persistent six-step guide, local agent command summary, appearance setup, existing-folder registration, one-level project-folder creation, inert setup-chat preparation, explicit generic ACP Connect/sign-in/questions, inline reviewed history import and Settings replay | Provider-specific enablement/terminal login breadth and live-provider fresh-install acceptance |
...
| iOS Simulator / device | Simulator discovery, boot/shutdown, screenshot capture, user-triggered HTTP(S) URL opening, app launch/termination and reviewed local .app installation in a selected booted simulator | Live stream/input, swipe/type/buttons, recording, accessibility tree and element targeting, and real macOS acceptance |
| Editor and diff review | Native multi-tab editor, literal find/replace, Markdown preview, changed-file/diff-row navigation, Explorer and Git review, explicit reload/overwrite actions after a save conflict, per-buffer opt-in local idle auto-save and committed-file history/revision preview without checkout | Syntax highlighting, cross-platform/remote autosave acceptance, richer conflict recovery, compare scopes, blame and deeper diff editing |
| Managed worktree isolation | Git worktree list/add/remove and review; assistant-turn forks can choose an existing linked worktree or create a reviewed local branch/worktree at a pinned commit with persisted task cwd and task-aware removal guard | Automatic cleanup/recovery, SSH managed creation and broader fork/orchestration environment choices |
| Provider/model/context controls | Generic ACP configuration, advertised-order model cycling by buttons, scoped keyboard and commands, persisted live-advertised model+effort presets and provider ordering for ACP agents, plus direct-model favorites with reviewed switching | Fast/thinking preset variants, direct-model keyboard parity, richer context/compaction UX and provider/account telemetry |
...
| Slash commands / keyboard control | Qualified `/synara/` workflow commands including goal set/edit/resume/pause/clear and saved automation review by exact ID, provider-advertised commands, command palette and constrained navigation remaps | Other argument forms, upstream interaction semantics and context-aware custom keybindings |
...
| Provider handoff / forks | Reviewed related unsent continuation and context-derived branches, with existing-worktree selection or reviewed new local worktree creation for assistant-turn forks | Same-task handoff semantics, provider-native fork paths and broader environment lifecycle |
| Attachments / transcript media | Durable still PNG/JPEG/WebP and text intake, bounded one-level folder snapshots and transcript images; WebP converts to bounded PNG for prompts, and local Hub Library PDFs have bounded page viewing | Persistent folder references, binary PDF prompt intake, broader document formats, Explorer/remote PDF viewing and platform acceptance |
| Studio / long-running output work | Native Studio/Hubs, file/output preview, reporting-turn attribution/filtering, PDF page viewing, reviewed original file export and output reopening in its reporting chat's Library | Historical per-turn file content capture, lifecycle depth, organization and long-running workflow polish |
...
- **Workspace file/source search:** native Explorer project-wide file-name and content search,
  ...
  fall back to file-only results. Debounced/coalesced queries and narrow-pane pointer targets now exist. Exact
  upstream ranking, SSH interaction and wider platform acceptance remain bounded deltas.
