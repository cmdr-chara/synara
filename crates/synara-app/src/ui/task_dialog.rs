...
use gpui::{Entity, EventEmitter, FocusHandle, Focusable, Subscription};
...
impl TaskDialog {
    pub fn new(
        ...
        let project = projects.iter().position(|(id, _)| Some(*id) == initial_project).unwrap_or(0);
        let agent = agents.iter().position(|(id, _, _)| Some(id.as_str()) == default_agent).unwrap_or(0);
        Self {
            prompt, projects, agents, project, agent, draft,
            busy: false, discard: false, error: None,
            popup: None, popup_subscription: None, needs_focus: true,
            _prompt_subscription: subscription,
        }
    }
    ...
    fn create(&mut self, cx: &mut Context<Self>) {
        if self.busy || self.discard || self.prompt.read(cx).text().trim().is_empty() { return; }
        let (Some((project, _)), Some((agent, _, _))) =
            (self.projects.get(self.project), self.agents.get(self.agent)) else { return; };
        let request = NewTaskRequest {
            project: *project, agent: agent.clone(),
            text: self.prompt.read(cx).text().to_owned(), send: !self.draft,
        };
        self.busy = true;
        self.error = None;
        self.popup = None;
    ...
    fn dismiss(&mut self, cx: &mut Context<Self>) {
        ...
        if self.has_text(cx) {
            ...
            self.popup = None;
            cx.notify();
        } else { cx.emit(TaskDialogEvent::Dismissed); }
    }
    fn open_field(&mut self, field: Field, window: &mut Window, cx: &mut Context<Self>) {
        if self.busy || self.discard { return; }
        let choices = match field {
            Field::Project => self.projects.iter().enumerate().map(|(index, (_, name))| menu::Choice {
                label: name.clone(), selected: index == self.project,
                icon: Some(Glyph::Folder), ..Default::default()
            }).collect(),
            Field::Agent => self.agents.iter().enumerate().map(|(index, (_, name, icon))| menu::Choice {
                label: name.clone(), selected: index == self.agent, icon: Some(*icon),
                detail: "Use this agent's default model. Configure models in the task.".into(),
                ..Default::default()
            }).collect(),
        };
        let popup = cx.new(|cx| menu::ChoiceMenu::new(
            match field { Field::Project => "Project", Field::Agent => "Agent" }.into(), choices, cx));
...
impl gpui::Render for TaskDialog {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let can_create = !self.busy && !self.discard && !self.projects.is_empty()
            && !self.agents.is_empty() && !self.prompt.read(cx).text().trim().is_empty();
        let project = self.projects.get(self.project).map_or("Choose project", |(_, name)| name.as_str());
        let (agent, glyph) = self.agents.get(self.agent)
            .map_or(("No configured agent", Glyph::Agent), |(_, name, glyph)| (name.as_str(), *glyph));
        let modal = div().id("new-task-dialog").role(gpui::Role::Dialog).aria_label("New task")
            ...
            .child(div().px_4().pb_2().text_size(px(11.)).text_color(rgb(palette().muted))
                .child("Agent default model. Model and session options are available after connecting in the task."))
        ...
        let modal = if cx.reduce_motion() { modal.into_any_element() } else {
            use gpui::AnimationExt;
            modal.with_animation("new-task-entry", gpui::Animation::new(std::time::Duration::from_millis(150))
                .with_easing(motion::ease_out), |element, value| element.opacity(value)).into_any_element()
        };
        div().absolute().inset_0().size_full().px_4().flex().items_center().justify_center()
            .bg(rgba(0x00000088)).occlude()
