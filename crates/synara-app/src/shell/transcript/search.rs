...
impl TranscriptState {
    pub fn jump_to_message(&self, thread: &Thread, anchor: &MessageAnchor) -> bool {
        if self.thread != Some(thread.id) { return false; }
        ...
        let index = self.indices.get(&direct).copied().or_else(|| {
            ...
            }).and_then(|turn| self.indices.get(&RowKey::Activity(turn.id.clone())).copied())
        });
...
mod tests {
    ...
    fn search_targets_visible_or_grouped_rows_with_role_aware_identity() {
        ...
        let events = [
            ...
            ThreadEvent::TextDelta { message_id: Some("shared".into()), role: Role::User, text: "Question".into() },
            ThreadEvent::TextDelta { message_id: Some("shared".into()), role: Role::Reasoning, text: "Thinking".into() },
            ThreadEvent::TextDelta { message_id: Some("answer".into()), role: Role::Assistant, text: "Answer".into() },
            ThreadEvent::PromptFinished { reason: "end_turn".into() },
        ];
        for (index, event) in events.into_iter().enumerate() {
            let envelope = EventEnvelope { id: EventId::new(), thread_id: thread.id, sequence: index as u64 + 1, timestamp_ms: index as i64, event };
            thread.apply(&envelope).unwrap();
            state.sync(&thread, Some(&envelope.event));
        }
        assert!(state.jump_to_message(&thread, &MessageAnchor { id: "shared".into(), role: Role::Reasoning }));
        assert_eq!(state.rows[state.list.logical_scroll_top().item_ix], RowKey::Activity("one".into()));
        assert!(!state.is_following());
        assert!(state.jump_to_message(&thread, &MessageAnchor { id: "shared".into(), role: Role::User }));
        assert_eq!(state.rows[state.list.logical_scroll_top().item_ix], RowKey::Message("shared".into(), 0));
        assert!(!state.jump_to_message(&thread, &MessageAnchor { id: "shared".into(), role: Role::Assistant }));
        assert!(!state.jump_to_message(&Thread::new(ThreadId::new()), &MessageAnchor { id: "shared".into(), role: Role::User }));
        state.follow();
        assert!(state.is_following());
    }
}
