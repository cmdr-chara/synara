...
impl NativeTerminal {
    ...
    pub fn spawn(
        ...
        launch.validate()?;
        if rows == 0 || columns == 0 || rows > 500 || columns > 1000 || !cwd.is_absolute() {
            return Err(RuntimeError::Invalid(
                "invalid terminal size or directory".into(),
            ));
        }
        ...
        let output = Arc::new(Mutex::new(TerminalOutput {
            ...
            parser: vt100::Parser::new(rows, columns, 5_000),
            revision: 0,
            error: None,
        }));
        ...
        std::thread::spawn(move || {
            ...
            loop {
                match reader.read(&mut buffer) {
                    Ok(0) => break,
                    Ok(n) => {
                        let Ok(mut out) = read_output.lock() else {
                            break;
                        };
                        out.tail.push(&buffer[..n]);
                        out.parser.process(&buffer[..n]);
                        out.revision += 1;
                    }
                    Err(error) => {
                        #[cfg(unix)]
        ...
        std::thread::spawn(move || {
            ...
            match result {
                Ok(status) => {
                    let _ = exit_tx.send(Some(status.exit_code()));
                }
                Err(error) => {
                    if let Ok(mut out) = wait_output.lock() {
                        out.error = Some(error.to_string());
                        out.revision += 1;
                    }
    ...
    pub fn resize(&self, rows: u16, columns: u16) -> Result<(), RuntimeError> {
        if rows == 0 || columns == 0 || rows > 500 || columns > 1000 {
            return Err(RuntimeError::Invalid("invalid terminal size".into()));
        }
        self.master
            .lock()
        ...
        output.parser.screen_mut().set_size(rows, columns);
        output.revision += 1;
        Ok(())
    }
    pub fn snapshot(&self) -> Result<TerminalSnapshot, RuntimeError> {
        ...
        Ok(TerminalSnapshot {
            text: output.parser.screen().contents(),
            raw_tail: output.tail.bytes(),
            truncated: output.tail.truncated(),
