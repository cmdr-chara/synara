...
impl DeviceCapture {
    pub fn decode(png: Vec<u8>) -> WorkspaceResult<Self> {
        if png.len() > 32 * 1024 * 1024 || !png.starts_with(b"\x89PNG\r\n\x1a\n") {
            return Err(WorkspaceError::Invalid("Invalid or oversized device PNG".into()));
        }
        ...
        let image = reader.decode().map_err(|_| WorkspaceError::Invalid("Device capture could not be decoded within the image limits".into()))?;
        ...
        DeviceFrame::new(width, height, FrameFormat::Rgba8, image.into_rgba8().into_raw(), 0)?;
        Ok(Self { png, width, height })
    }
    pub fn orientation(&self) -> &'static str {
        if self.width > self.height { "Landscape" } else if self.height > self.width { "Portrait" } else { "Square" }
    }
}
...
pub fn device_viewport_point(x: f32, y: f32, view_width: f32, view_height: f32, width: u32, height: u32) -> Option<(u32, u32)> {
    if [x, y, view_width, view_height].iter().any(|n| !n.is_finite()) || view_width <= 0. || view_height <= 0. || width == 0 || height == 0 {
        return None;
    }
    ...
    (x >= 0. && y >= 0. && x < width as f32 && y < height as f32).then(|| (x.floor() as u32, y.floor() as u32))
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn rejects_letterbox_edges_nonfinite_and_zero_sized_viewports() {
        assert_eq!(device_viewport_point(100., 100., 200., 200., 100, 200), Some((50, 100)));
        for (x, y) in [(0., 100.), (200., 100.), (150., 100.), (100., 200.), (f32::NAN, 0.)] {
            assert_eq!(device_viewport_point(x, y, 200., 200., 100, 200), None);
        }
        assert_eq!(device_viewport_point(0., 0., 0., 0., 100, 200), None);
        assert_eq!(device_viewport_point(50., 0., 200., 200., 100, 200), Some((0, 0)));
    }
    #[test]
    fn capture_decode_validates_pixels_and_orientation_not_just_header() {
        let mut png = Cursor::new(Vec::new());
        image::DynamicImage::ImageRgba8(image::RgbaImage::new(2, 3)).write_to(&mut png, ImageFormat::Png).unwrap();
        let capture = DeviceCapture::decode(png.into_inner()).unwrap();
        assert_eq!((capture.width, capture.height, capture.orientation()), (2, 3, "Portrait"));
