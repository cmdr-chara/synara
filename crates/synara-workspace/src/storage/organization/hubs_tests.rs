...
async fn legacy_studio_is_a_read_only_hub_and_normal_chats_stay_separate() {
    ...
    service
        .access(move |store| {
            assert!(
                store
                    .preference_raw(&format!("hub:{}", project.id))?
                    .is_none()
            );
            Ok(())
        })
...
async fn hub_creation_context_conflicts_and_archive_preserve_data_on_reopen() {
    ...
    let agent = service.profiles().await.unwrap()[0].id.clone();
    let (mut hub, main) = service
        .create_hub(root.clone(), "Research".into(), agent.clone())
        .await
        .unwrap();
    assert!(
        service
            .create_hub(root, "Duplicate".into(), agent.clone())
            .await
            .is_err()
    );
    hub.instructions = "Use primary sources".into();
    hub.memory = "Keep 日本語 references".into();
    let stale = hub.clone();
    hub = service.save_hub(0, hub).await.unwrap();
    assert!(service.save_hub(0, stale).await.is_err());
    let thread = service
        .create_hub_thread(hub.project, "New task".into(), agent.clone())
        .await
        .unwrap();
    assert_eq!(
        service.task_draft(thread.id).await.unwrap(),
        hub.context_draft()
    );
    ...
    assert!(
        service
            .create_hub_thread(hub.project, "No".into(), agent)
            .await
            .is_err()
    );
...
async fn malformed_hub_metadata_is_not_replaced_by_legacy_defaults() {
    ...
    let agent = service.profiles().await.unwrap()[0].id.clone();
    let (hub, _) = service
        .create_hub(dir.path().into(), "Keep".into(), agent)
        .await
        .unwrap();
        ...
        .access(move |store| {
            store
                ...
                .execute(
                    ...
                    rusqlite::params!["{\"version\":99}", format!("hub:{project}")],
                )
    ...
    service
        .access(move |store| {
            assert_eq!(
                store.preference_raw(&format!("hub:{project}"))?.as_deref(),
                Some("{\"version\":99}")
            );
            Ok(())
        })
