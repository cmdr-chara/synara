...
fn headers(head: &str, host: &str, token: &str) -> std::result::Result<usize, u16> {
    ...
    if !matches!(
        ...
        None | Some(&"2025-03-26") | Some(&"2025-06-18")
    ) {
        return Err(400);
    }
    if map
...
pub(super) fn read(
    ...
    let end = loop {
        ...
        let n = socket.read(&mut buf).map_err(|_| 408u16)?;
        if n == 0 {
            return Err(400);
        }
...
mod tests {
    ...
    #[test]
    fn strict_http_authority_and_bounds() {
        ...
        assert_eq!(
            headers(
                &head("").replace("Content-Length: 2", "Content-Length: 17000"),
                "127.0.0.1:123",
                "token"
            ),
            Err(413)
    ...
    #[test]
    fn constant_time_comparison_requires_exact_token() {
