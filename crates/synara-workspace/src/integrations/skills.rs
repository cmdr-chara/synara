...
impl SkillReview {
    pub fn document(&self) -> &InstalledSkill {
        &self.skill
    }
    pub fn inspect(path: PathBuf) -> WorkspaceResult<Self> {
        ...
        {
            return Err(invalid(
                "Choose an absolute path to a Markdown (.md) skill document.",
            ));
        }
        for part in path.ancestors() {
            if fs::symlink_metadata(part)
                .map_err(|_| invalid("The selected skill path is unavailable."))?
                .file_type()
                .is_symlink()
            {
        ...
        let meta = fs::symlink_metadata(&path)
            .map_err(|_| invalid("Cannot inspect the selected skill."))?;
        if !meta.is_file() || meta.len() > MAX_SKILL_BYTES as u64 {
            return Err(invalid(
                "Choose a regular Markdown file no larger than 64 KiB.",
            ));
        }
        let mut bytes = Vec::new();
        fs::File::open(&path)
            .map_err(|_| invalid("Cannot open the selected skill."))?
            .take(MAX_SKILL_BYTES as u64 + 1)
            .read_to_end(&mut bytes)
            .map_err(|_| invalid("Cannot read the selected skill."))?;
        let markdown =
            String::from_utf8(bytes).map_err(|_| invalid("Skill documents must be UTF-8 text."))?;
        let mut title = path
            .file_stem()
            .and_then(|p| p.to_str())
            .unwrap_or("Skill document")
            .to_owned();
        ...
        let skill = InstalledSkill {
            ...
            origin: SkillOrigin {
                path: path
                    .to_str()
                    .ok_or_else(|| invalid("Skill paths must be UTF-8."))?
                    .into(),
                sha256: digest(&markdown),
                version,
            },
            previous_origins: vec![],
            markdown,
            enabled: false,
        };
        skill.validate()?;
    ...
    pub(crate) fn recheck(self) -> WorkspaceResult<InstalledSkill> {
        ...
        if current.skill.origin != self.skill.origin {
            return Err(invalid(
                "The skill changed after review. Review the new document before installing.",
            ));
        }
        Ok(self.skill)
    }
