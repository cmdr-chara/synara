...
impl Default for Personalization {
    fn default() -> Self {
        Self {
            version: 1,
            zen_mode: false,
            colorway: Colorway::Original,
            material: SurfaceMaterial::Solid,
            canvas_opacity: 85,
            panel_opacity: 92,
            wallpaper: None,
            wallpaper_dim: 55,
            wallpaper_blur: 24,
            wallpaper_fit: WallpaperFit::Cover,
...
impl Personalization {
    pub fn validate(&self) -> WorkspaceResult<()> {
        if self.version != 1
            || !(35..=100).contains(&self.canvas_opacity)
            || !(60..=100).contains(&self.panel_opacity)
            || self.wallpaper_dim > 95
            || self.wallpaper_blur > 64
            ...
            || self.accent.is_some_and(|value| value > 0xffffff)
        {
            return Err(WorkspaceError::Invalid(
                "Invalid appearance values or unsupported personalization version.".into(),
            ));
        }
...
fn validate_wallpaper_path(path: &std::path::Path) -> WorkspaceResult<()> {
    let text = path
        .to_str()
        .ok_or_else(|| WorkspaceError::Invalid("Choose an image with a UTF-8 file path.".into()))?;
    if !path.is_absolute()
        || text.len() > 8192
    ...
    {
        return Err(WorkspaceError::Invalid(
            "Wallpaper must be a local absolute file path, not a URL or network share.".into(),
        ));
    }
    Ok(())
}
...
impl AppearanceProfile {
    ...
    pub fn import(text: &str, current: &AppearanceSettings) -> WorkspaceResult<AppearanceSettings> {
        if text.len() > 64 * 1024 {
            return Err(WorkspaceError::Invalid(
                "Appearance profiles are limited to 64 KiB.".into(),
            ));
        }
        let mut profile: Self = serde_json::from_str(text).map_err(|error| {
            WorkspaceError::Invalid(format!("Invalid appearance profile: {error}"))
        })?;
        if profile.version != 1 {
            return Err(WorkspaceError::Invalid(
                "Unsupported appearance profile version. Settings were not changed.".into(),
            ));
        }
...
impl WorkspaceService {
    ...
    pub async fn render_wallpaper(path: PathBuf, blur: u8) -> WorkspaceResult<WallpaperAsset> {
        validate_wallpaper_path(&path)?;
        if blur > 64 {
            return Err(WorkspaceError::Invalid("Wallpaper blur exceeds 64.".into()));
        }
