...
pub fn desktop_notification_status() -> &'static str {
    if cfg!(target_os = "linux") && Path::new("/usr/bin/notify-send").is_file() && std::env::var_os("DBUS_SESSION_BUS_ADDRESS").is_some() {
...
pub fn desktop_notifications_available() -> bool {
    (cfg!(target_os = "linux") && Path::new("/usr/bin/notify-send").is_file() && std::env::var_os("DBUS_SESSION_BUS_ADDRESS").is_some())
...
pub async fn desktop_notify(request: NotificationRequest) -> Result<(), RuntimeError> {
    request.validate()?;
    if !desktop_notifications_available() { return Err(RuntimeError::Unsupported(desktop_notification_status().into())); }
    let (command, args) = if cfg!(target_os = "macos") {
        ...
        ("/usr/bin/notify-send", vec!["--app-name=Synara".into(), "--".into(), request.title, request.body])
    };
    crate::device_tools::command::run(Path::new(command), args, 4096, &CancellationToken::new()).await?;
    Ok(())
}
