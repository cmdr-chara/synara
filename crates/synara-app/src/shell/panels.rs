        ...
        div().flex().flex_col().flex_1().min_h_0().min_w_0()
            ...
            .child(div().flex().flex_1().min_h_0().min_w_0().when(narrow, |el| el.flex_col())
                .children(self.editors.tree_visible.then(|| div().relative().child(ui::layout_probe("file-tree"))
                    .when(narrow, |el| el.h(px(if self.explorer.search_open { 330. } else { 185. })).w_full().border_b_1())
                    ...
                    .child(div().flex_shrink_0().px_2().pb_2().flex().items_center().gap_2()
                ...
                .child(if let Some(document) = &self.document {
                    div().flex_1().min_w_0().min_h_0().flex().flex_col().p_3().gap_2()
                        ...
                        .child(self.editor_history_content(cx).unwrap_or_else(|| if self.editors.preview && self.editor_preview_available() { self.editor_preview(cx) } else { self.editor_content() }))
