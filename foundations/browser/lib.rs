...
impl BrowserPolicy {
    ...
    fn prune(&mut self, now_ms: u64) {
        self.pending
            .retain(|_, prompt| now_ms < prompt.expires_at_ms);
        self.grants
            .retain(|_, prompt| now_ms < prompt.expires_at_ms);
    }
...
mod tests {
    ...
    fn consent_is_one_shot_and_never_implicit() {
        ...
        assert_eq!(
            policy.consume(grant, tab, 7, &Action::Screenshot, 2),
            Ok(())
        );
        assert_eq!(
            policy.consume(grant, tab, 7, &Action::Screenshot, 3),
            Err(Error::Stale)
        );
    }
    #[test]
    ...
    fn browsing_authentication_and_other_tasks_are_isolated() {
        ...
        for context in [
            Context::Manual,
            Context::Authentication(7),
            Context::AgentTask(8),
        ] {
            let tab = loaded(&mut policy, context);
            assert_eq!(
                policy.request(tab, 7, Action::ReadDocument, 0).unwrap_err(),
                Error::WrongContext
            );
        }
    }
    ...
    fn grant_cannot_change_action_task_or_tab() {
        ...
        for (target, task, action) in [
            (other, 7, Action::Screenshot),
            (tab, 8, Action::Screenshot),
            (tab, 7, Action::Input),
        ] {
            let grant = approved(&mut policy, tab);
            assert_eq!(
                policy.consume(grant, target, task, &action, 2),
                Err(Error::Denied)
            );
        }
    }
    ...
    fn stale_navigation_cannot_replace_the_current_document() {
        ...
        assert_eq!(
            policy.commit_navigation(current, origin()),
            Err(Error::Stale)
        );
    }
    #[test]
    fn loading_and_crashed_tabs_have_no_usable_document() {
        ...
        let navigation = policy.begin_navigation(tab).unwrap();
        assert_eq!(
            policy.request(tab, 7, Action::Screenshot, 0).unwrap_err(),
            Error::NotReady
        );
        policy.crash(tab).unwrap();
        assert_eq!(
            policy.commit_navigation(navigation, origin()),
            Err(Error::Stale)
        );
        assert_eq!(
            policy.request(tab, 7, Action::Screenshot, 0).unwrap_err(),
    ...
    fn closing_a_tab_revokes_grants_without_reusing_its_identity() {
        ...
        assert_eq!(
            policy.consume(grant, replacement, 7, &Action::Screenshot, 2),
            Err(Error::Stale)
        );
    }
    #[test]
    fn independent_tabs_keep_independent_authority() {
        ...
        assert_eq!(
            policy.consume(grant, second, 7, &Action::Screenshot, 2),
            Ok(())
        );
    }
    #[test]
    fn expiry_is_checked_at_approval_and_dispatch() {
        ...
        let prompt = policy.request(tab, 7, Action::Input, 0).unwrap();
        assert_eq!(
            policy.resolve(prompt.id, true, CONSENT_TTL_MS),
            Err(Error::Expired)
        );
        let grant = approved(&mut policy, tab);
        assert_eq!(
            policy.consume(grant, tab, 7, &Action::Screenshot, CONSENT_TTL_MS),
            Err(Error::Expired)
        );
    }
    #[test]
    fn pending_and_approved_consent_share_a_resource_limit() {
        ...
        assert_eq!(
            policy.request(tab, 7, Action::Input, 2).unwrap_err(),
            Error::Limit
        );
        assert!(
            policy
                .request(tab, 7, Action::Input, CONSENT_TTL_MS)
                .is_ok()
        );
    ...
    fn tab_limit_and_blank_document_are_explicit() {
        ...
        let tab = policy.open_tab(Context::AgentTask(7)).unwrap();
        assert_eq!(
            policy.request(tab, 7, Action::Input, 0).unwrap_err(),
            Error::NotReady
        );
        for _ in 1..MAX_TABS {
            policy.open_tab(Context::Manual).unwrap();
        }
    ...
    fn origin_parts_are_not_url_or_script_inputs() {
        for host in [
            ...
            assert_eq!(
                Origin::from_canonical_parts(Scheme::Https, host, 443),
