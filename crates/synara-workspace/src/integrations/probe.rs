...
pub(crate) async fn credential(
...
) -> WorkspaceResult<Option<SecretValue>> {
    let Some(reference) = &config.bearer else {
        return Ok(None);
    };
    match store.state() {
        SecretStoreState::Locked => {
            return Err(invalid(
                "The credential store is locked. Unlock it before testing or sharing this connection.",
            ));
        }
    ...
    let value = store
        ...
        .await
        .map_err(|_| {
            invalid("Cannot read this credential-store reference. No credential was persisted.")
        })?
    ...
    let token = std::str::from_utf8(value.expose())
        .map_err(|_| invalid("The referenced bearer credential is not valid text."))?;
    ...
    {
        return Err(invalid(
            "The referenced bearer credential is not a valid HTTP bearer token.",
        ));
    }
    Ok(Some(value))
}
...
impl Probe {
    ...
    fn post(&mut self, method: &str, id: Option<u64>, mut params: Value) -> WorkspaceResult<Reply> {
        ...
        let mut response = request.send(message.to_string()).map_err(|_| {
            invalid("MCP network, TLS or timeout failure. No connection was confirmed.")
        })?;
        let status = response.status().as_u16();
        if status == 401 || status == 403 {
            return Err(invalid(
                "MCP authentication was rejected. Check the credential reference and provider authorization.",
            ));
        }
        ...
        if id.is_none() {
            return if status == 202 {
                Ok(Reply { status, rpc: None })
            } else {
                Err(invalid("MCP initialization notification was not accepted."))
            };
        }
        let mime = response
            .headers()
        ...
        let rpc = match mime.as_str() {
            "application/json" => {
                ...
                if bytes.len() as u64 > MAX_BODY {
                    return Err(invalid("MCP response exceeds 1 MiB."));
                }
                serde_json::from_slice(&bytes).ok()
            }
            "text/event-stream" => Some(read_sse(
                response.body_mut().as_reader(),
                id.expect("request id"),
            )?),
            _ if matches!(status, 400 | 404 | 405) => None,
            _ => {
                return Err(invalid(
                    "The endpoint did not return MCP JSON or an event stream.",
                ));
            }
        };
        Ok(Reply { status, rpc })
    ...
    fn result(reply: Reply, id: u64) -> WorkspaceResult<Value> {
        if !(200..300).contains(&reply.status) {
            return Err(invalid(&format!(
                "MCP HTTP {}. No connection was confirmed.",
                reply.status
            )));
        }
        let rpc = reply
            .rpc
            .ok_or_else(|| invalid("Invalid MCP JSON response."))?;
        ...
        let result = rpc
            ...
            .ok_or_else(|| invalid("MCP response has no result object."))?;
        if result
            .get("resultType")
            .is_some_and(|v| v.as_str() != Some("complete"))
        {
            return Err(invalid(
                "MCP discovery requires an unsupported interaction. No callbacks or tool execution were authorized.",
            ));
        }
        Ok(result.clone())
    }
    fn discover(&mut self) -> WorkspaceResult<McpProbeReport> {
        ...
        let result = if fallback {
            ...
            let version = result
                ...
                .ok_or_else(|| invalid("MCP initialization omitted its protocol version."))?;
            ...
            if !result
                ...
                .is_some_and(|versions| versions.iter().any(|v| v.as_str() == Some(MODERN)))
            {
                return Err(invalid(
                    "The server did not advertise the requested MCP protocol version.",
                ));
            }
            result
        };
        let capabilities = result
            ...
            .ok_or_else(|| invalid("MCP discovery omitted capability negotiation."))?;
        if capabilities.len() > 64 {
            return Err(invalid("Too many MCP capabilities."));
        }
        let server = if self.protocol == MODERN {
            result.pointer("/_meta/io.modelcontextprotocol~1serverInfo/name")
        } else {
            result.pointer("/serverInfo/name")
        };
        let mut report = McpProbeReport {
            protocol: self.protocol.clone(),
            server: self.display(server, 160),
            capabilities: capabilities
                .keys()
                .map(|k| self.display(Some(&Value::String(k.clone())), 128))
                .collect(),
            tools: vec![],
            elapsed_ms: 0,
        ...
        if capabilities.contains_key("tools") {
            if !capabilities["tools"].is_object() {
                return Err(invalid("Invalid MCP tools capability."));
            }
            let mut cursor = None;
            ...
            for page in 0..4 {
                ...
                let tools = result
                    .get("tools")
                    .and_then(Value::as_array)
                    .ok_or_else(|| invalid("MCP tool list is malformed."))?;
                if report.tools.len() + tools.len() > 256 {
                    return Err(invalid("MCP discovery is limited to 256 tools."));
                }
                for tool in tools {
                    let name = tool
                        .get("name")
                        .and_then(Value::as_str)
                        .filter(|name| text(name, 256))
                    ...
                    report.tools.push(McpToolInfo {
                        name: self.display(tool.get("name"), 256),
                        description: self.display(tool.get("description"), 1024),
                    });
                }
                cursor = match result.get("nextCursor") {
                    ...
                    Some(value) => Some(
                        value
                            .as_str()
                            .filter(|v| text(v, 1024))
                            .ok_or_else(|| invalid("Invalid MCP pagination cursor."))?
                            .to_string(),
                    ),
                };
                let Some(next) = cursor.as_ref() else { break };
                if page == 3 || !seen.insert(next.clone()) {
                    return Err(invalid(
                        "MCP pagination exceeded its bound or repeated a cursor.",
                    ));
                }
            }
        }
    ...
    fn display(&self, value: Option<&Value>, limit: usize) -> String {
        ...
        let value = self
            .token
            .as_ref()
            .and_then(|secret| std::str::from_utf8(secret.expose()).ok())
            .map_or_else(
                || value.to_owned(),
...
fn read_sse(reader: impl Read, id: u64) -> WorkspaceResult<Value> {
    ...
    loop {
        ...
        let n = reader
            .read_line(&mut line)
            .map_err(|_| invalid("Invalid or timed-out MCP event stream."))?;
        total += n;
        if total as u64 > MAX_BODY {
            return Err(invalid("MCP event stream exceeds 1 MiB."));
        }
        if n == 0 {
            return Err(invalid("MCP event stream ended without a matching result."));
        }
        if line.trim_end_matches(['\r', '\n']).is_empty() {
            if !data.is_empty() {
                let rpc: Value = serde_json::from_str(&data)
                    .map_err(|_| invalid("Malformed MCP event data."))?;
                if rpc.get("id") == Some(&json!(id)) && rpc.get("method").is_none() {
                    return Ok(rpc);
                }
                if rpc.get("method").is_some() && rpc.get("id").is_some() {
                    return Err(invalid("MCP requested an unauthorized client callback."));
                }
                data.clear();
            }
...
pub(crate) fn probe(
    config: ManagedMcp,
    token: Option<SecretValue>,
) -> WorkspaceResult<McpProbeReport> {
    config.validate()?;
    ...
    let mut probe = Probe {
        http,
        url: endpoint(&config.endpoint)?.into(),
        token,
        session: None,
    ...
    result.map(|mut report| {
        report.cleanup_warning = !cleanup;
        report
    })
}
