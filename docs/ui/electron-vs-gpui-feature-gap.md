...
| Attachments/media | Persistent folder references; still PNG/JPEG/WebP and UTF-8 text; bounded PDF, DOCX, ODT, ODP, ODS, PPTX and XLSX intake with inert prompt projection; Studio document previews/original export; PDF paging/text/HTTP(S) links/form-type disclosure | Safe PDF field-level interaction plus cross-platform/package and wider live failure acceptance |
