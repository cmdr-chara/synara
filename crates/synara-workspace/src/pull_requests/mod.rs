...
struct StopOnDrop(synara_runtime::ProcessHandle);
...
impl GithubRepository {
    pub fn parse(remote: &str) -> Result<Self> {
        ...
        let (owner, name) = path
            .split_once('/')
            .ok_or("Repository must contain owner/name")?;
        let valid = |s: &str| {
            !s.is_empty()
                && s.len() <= 100
                && s != "."
...
impl PullRequests {
    ...
    pub async fn discover(
    ...
    ) -> Result<Vec<(String, GithubRepository)>> {
        ...
        if names.lines().count() > 16 {
            return Err(
                "More than 16 remotes. Select a smaller repository configuration explicitly."
                    .into(),
            );
        }
        let mut repos = vec![];
        for name in names.lines() {
            ...
            if let Ok(repo) = GithubRepository::parse(url.trim()) {
                repos.push((name.to_owned(), repo));
            }
        }
        if repos.is_empty() {
            return Err(
                "No supported GitHub.com remote found. Other providers are not implemented.".into(),
            );
        }
        Ok(repos)
    }
    pub async fn list(
        ...
        repo.path()?;
        if text.len() > 256 || text.chars().any(char::is_control) || !(1..=20).contains(&page) {
            return Err("Search is limited to 256 characters and 20 pages.".into());
        }
        let state = match filter {
            PrFilter::Open => "is:open",
            PrFilter::Closed => "is:closed",
            PrFilter::Draft => "is:open draft:true",
            PrFilter::All => "",
        };
        ...
        let value = self
            .api(
                "GET",
                &format!("search/issues?{encoded}&sort=updated&order=desc&per_page=50&page={page}"),
                None,
                cancel,
            )
            .await?;
        if value["incomplete_results"].as_bool() == Some(true) {
            return Err(
                "GitHub returned incomplete search results. Narrow the search and refresh.".into(),
            );
        }
        decode_list(
            value
    ...
    pub async fn detail(
        ...
        let work = async {
            let pr: PullRequest =
                serde_json::from_value(self.api("GET", &path, None, cancel.clone()).await?)
                    .map_err(|_| "Invalid PR detail")?;
            ...
            for (name, endpoint) in [
                ("files", format!("{path}/files?per_page=100")),
                ("commits", format!("{path}/commits?per_page=100")),
                (
                    "activity",
                    format!("{}/issues/{number}/timeline?per_page=100", repo.path()?),
                ),
                ("reviews", format!("{path}/reviews?per_page=100")),
            ] {
                let result = self.api("GET", &endpoint, None, cancel.clone()).await;
                match result {
                    Ok(value) => match name {
                        "files" => match decode_list(value, 100) {
                            Ok(v) => out.files = v,
                            Err(e) => out.notices.push(format!("Files: {e}")),
                        },
                        _ => match decode_list(value, 100) {
                            Ok(v) => match name {
                                "commits" => out.commits = v,
                                "activity" => out.activity = v,
                                _ => out.reviews = v,
                            },
            ...
            if let Some(sha) = out.pr.head["sha"].as_str().filter(|s| valid_sha(s)) {
                for (name, endpoint, field) in [
                    (
                        "checks",
                        format!("{}/commits/{sha}/check-runs?per_page=100", repo.path()?),
                        "check_runs",
                    ),
                    (
                        "statuses",
                        format!("{}/commits/{sha}/status?per_page=100", repo.path()?),
                        "statuses",
                    ),
                ] {
                    match self
                        .api("GET", &endpoint, None, cancel.clone())
                        .await
                        .and_then(|v| decode_list(v[field].clone(), 100))
                    {
                        Ok(v) => {
                            if name == "checks" {
                                out.checks = v
        ...
        tokio::time::timeout(Duration::from_secs(60), work)
            .await
            .map_err(|_| "Detail request timed out".to_owned())?
    }
    ...
    async fn api(
        ...
        let operation = async {
            let process = tokio::select! {
                ...
                result = self.host.spawn(&launch, &self.root) => result.map_err(|_| "GitHub CLI unavailable on the selected host. Install/authenticate gh explicitly there.".to_owned())?,
            };
            let handle = process.handle;
            let _owner = StopOnDrop(handle.clone());
            let read = async {
                let writer = async {
                    ...
                    if let Some(body) = body {
                        stdin
                            .write_all(&body)
                            .await
                            .map_err(|_| "Cannot write request".to_owned())?;
                    }
                    stdin
                        .shutdown()
                        .await
                        .map_err(|_| "Cannot close request".to_owned())
                };
                ...
                serde_json::from_slice(&out)
                    .map_err(|_| "Invalid or oversized GitHub JSON response".to_owned())
            };
        ...
        tokio::time::timeout(Duration::from_secs(20), operation)
            .await
            .map_err(|_| "GitHub request timed out".to_owned())?
    }
}
async fn read_bounded(reader: synara_runtime::ProcessReader, max: usize) -> Result<Vec<u8>> {
    ...
    reader
        ...
        .await
        .map_err(|_| "Cannot read GitHub process")?;
    if output.len() > max {
        return Err("GitHub response exceeded its byte limit".into());
    }
    Ok(output)
}
fn request_plan(
    ...
    endpoint: &str,
    body: Option<Value>,
) -> Result<(LaunchSpec, Option<Vec<u8>>)> {
    if !matches!(method, "GET" | "POST" | "PATCH" | "PUT")
        || endpoint.len() > 2048
    ...
    let body = body
        .map(|v| serde_json::to_vec(&v).map_err(|_| "Invalid JSON"))
        .transpose()?;
    if body.as_ref().is_some_and(|v| v.len() > 128 * 1024) {
    ...
    if body.is_some() {
        spec.args.extend(["--input".into(), "-".into()]);
    }
...
fn decode_list<T: for<'de> Deserialize<'de>>(value: Value, max: usize) -> Result<Vec<T>> {
    if value.as_array().is_none_or(|v| v.len() > max) {
        return Err("Invalid or oversized collection".into());
    }
    serde_json::from_value(value).map_err(|_| "Invalid provider collection".into())
}
fn positive(number: u64) -> Result<u64> {
    if number == 0 {
        Err("Invalid PR number".into())
    } else {
        Ok(number)
    }
}
