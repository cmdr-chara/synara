...
use synara_agent::{AgentError, AgentResult, AgentSession, ConnectionInfo, SessionOptions};
use synara_browser::session::{Capabilities, Command, Event};
...
impl AgentConnection for Connection {
    ...
    async fn new_session(&self, _: SessionOptions) -> AgentResult<Arc<dyn AgentSession>> {
        Err(AgentError::Unsupported("fixture".into()))
    }
    async fn disconnect(&self) -> AgentResult<()> {
        self.0
            .send_modify(|v| v.state = ConnectionState::Disconnected);
        Ok(())
    }
}
...
async fn rpc(context: &ContextServer, method: &str, params: Value, extra: &str) -> (u16, Value) {
    let ContextServer::Http { url, headers, .. } = context else {
        panic!()
    };
    let host = url
        ...
        .to_owned();
    let auth = headers["Authorization"].clone();
...
async fn real_loopback_transport_queues_once_requires_native_approval_and_revokes() {
    ...
    let task = workspace
        .create_task(project.id, "Browser fixture".into(), profile.id.clone())
        .await
        .unwrap();
    let (tx, _) = tokio::sync::watch::channel(ConnectionInfo {
        id: ConnectionId::new(),
    ...
    let connection = Arc::new(Connection(tx));
    ...
    assert_eq!(
        rpc(&context, "tools/list", json!({}), "Origin: null\r\n")
            .await
            .0,
        403
    );
    let (_, tabs) = rpc(
        &context,
        "tools/call",
        json!({"name":"browser_tabs","arguments":{}}),
        "",
    )
    .await;
    ...
    let (_, repeat) = rpc(&context, "tools/call", params, "").await;
    assert_eq!(content(&repeat)["request"], request);
    assert_eq!(
        commands.lock().unwrap().len(),
        1,
        "No navigation before consent"
    );
    let (_, rejected) = rpc(
        &context,
        "tools/call",
        json!({"name":"browser_approve","arguments":{"request":request}}),
        "",
    )
    .await;
    ...
    let (_, result) = rpc(
        &context,
        "tools/call",
        json!({"name":"browser_result","arguments":{"request":request}}),
        "",
    )
    .await;
    assert_eq!(content(&result)["receipt"]["state"]["state"], "complete");
    let old = owner.bind(99).unwrap();
    let new = owner.bind(99).unwrap();
    owner.revoke_generation(99, old.generation);
    assert!(old.tabs().is_err());
    assert_eq!(new.tabs().unwrap().len(), 1);
    drop(endpoint);
    tokio::time::sleep(Duration::from_millis(100)).await;
    assert!(
        owner
            .with(|s, _| Ok(s.task_tabs(task.id.0.as_u128())))
            .unwrap()
            .is_empty()
    );
    owner.shutdown();
