...
#[test]
fn first_run_uses_distinct_electron_ui_and_code_base_sizes() {
    let settings = AppSettings::default();
    assert_eq!(settings.appearance.fonts.ui_size, 13.0);
    assert_eq!(settings.appearance.fonts.code_size, 12.0);
...
fn old_minimal_settings_receive_defaults_without_changing_the_schema_version() {
    ...
    assert_eq!(decoded.appearance.fonts.code_size, 12.0);
