...
impl Shell {
    ...
    pub(super) fn pr_fix_add(&mut self, cx: &mut Context<Self>) {
        ...
        let view = &mut self.pull_requests;
        let (Some(review), Some(client)) = (view.fix.clone(), view.client.clone()) else {
            return;
        };
        ...
        let draft = match review
            .review
            .draft(view.fix_text.read(cx).text(), &expected)
        {
            Ok(draft) => draft,
            Err(error) => {
    ...
    pub(super) fn pr_fix_validated(
        ...
        if Some(review.task) != self.selected
            ...
            || self.close != CloseState::Open
        {
            self.pull_requests.error=Some("Task or draft changed during validation. Nothing was added. Review and try again explicitly.".into());
            cx.notify();
            return;
        }
        self.composer
