...
impl IntegrationState {
    pub fn new(cx: &mut Context<Shell>) -> Self {
        let query = cx.new(|cx| {
            TextEntry::new(
                "Search name, description, origin or agent",
                EntryMode::SingleLine,
                32.,
                cx,
            )
        });
        let skill_path = cx.new(|cx| {
            TextEntry::new(
    ...
    pub fn pending(&self) -> bool {
        self.busy || self.form.is_some() || self.review.is_some() || self.confirm.is_some()
    }
    fn matches(&self, enabled: bool, fields: &[&str], cx: &Context<Shell>) -> bool {
        let query = self.query.read(cx).text().trim().to_lowercase();
        self.filter.is_none_or(|filter| filter == enabled)
            && fields
                .iter()
...
impl Shell {
    fn integration_job(
        ...
        if self.integrations.busy || self.close != CloseState::Open {
            return;
        }
    ...
    pub(super) fn integration_reply(&mut self, reply: Reply, cx: &mut Context<Self>) {
        ...
        match reply {
            ...
            Reply::Changed(value, message) => {
                ...
                self.integrations.review = None;
                self.integrations.preview = None;
                self.integrations.form = None;
                self.integrations.confirm = None;
                self.integrations.reports.clear();
                self.poll();
            }
            Reply::Reviewed(review, replace) => {
                self.integrations.preview = None;
                self.integrations.review = Some((review, replace));
            }
                    ...
                    || self.composer.read(cx).is_composing()
                {
                    ...
                    if combined.len() > 1024 * 1024 {
                        self.integrations.error =
                            Some("The combined draft exceeds 1 MiB. Nothing was inserted.".into());
                    } else {
                        self.composer
            ...
            Reply::Disconnected => {
                self.integrations.confirm = None;
                self.integrations.notice=Some("The shared agent process was disconnected. No task was restarted. Retry the configuration change.".into());
                self.poll();
            }
    ...
    fn choose_skill(&mut self, replace: Option<String>, cx: &mut Context<Self>) {
        ...
        let picker = cx.prompt_for_paths(gpui::PathPromptOptions {
            ...
            directories: false,
            multiple: false,
            prompt: Some("Review a Markdown skill document".into()),
        });
            ...
            let _ = view.update(cx, |this, cx| {
                ...
                match result {
                    Ok(Ok(Some(paths))) if paths.len() == 1 => {
                        ...
                        this.integration_job(
                            async move {
                                workspace
                                    .review_skill(path)
                                    .await
                                    .map(|review| Reply::Reviewed(review, replace))
                                    .map_err(|e| e.to_string())
                            },
                            cx,
                        );
                    ...
                    Ok(Ok(None)) => {}
                    _ => this.integrations.error = Some(
                        "The native skill file picker was unavailable or did not return one file."
                            .into(),
                    ),
                }
                cx.notify();
    ...
    fn review_skill_path(&mut self, cx: &mut Context<Self>) {
        if self.integrations.skill_path.read(cx).is_composing() {
            return;
        }
        ...
        self.integration_job(
            async move {
                workspace
                    .review_skill(path)
                    .await
                    .map(|review| Reply::Reviewed(review, None))
                    .map_err(|e| e.to_string())
            },
            cx,
        );
    ...
    fn approve_skill(&mut self, cx: &mut Context<Self>) {
        let Some(value) = &self.integrations.value else {
            return;
        };
        let Some((review, replace)) = self.integrations.review.clone() else {
            return;
        };
    ...
    fn insert_skill(&mut self, id: String, cx: &mut Context<Self>) {
        ...
        let revision = value.revision;
        let generation = self.selection_revision;
        let workspace = self.controller.workspace.clone();
        self.integration_job(
            async move {
                let value = workspace.integrations().await.map_err(|e| e.to_string())?;
                if value.revision != revision {
                    return Err("The skill library changed. Reload before inserting.".into());
                }
                let skill = value
                    .skills
                    .into_iter()
                    .find(|skill| skill.id == id && skill.enabled)
                    .ok_or("This skill is disabled or no longer installed.")?;
                Ok(Reply::Insert(
                    task,
                    generation,
    ...
    fn edit_mcp_form(&mut self, config: Option<ManagedMcp>, cx: &mut Context<Self>) {
        ...
        let config = match config {
            ...
            None => {
                let Some(task) = self.task() else {
                    self.integrations.error =
                        Some("Select a task and agent before adding an MCP connection.".into());
                    cx.notify();
                    return;
                };
        ...
        self.integrations.form = Some(McpForm {
            config,
            name,
            endpoint,
            service,
    ...
    fn save_mcp_form(&mut self, cx: &mut Context<Self>) {
        let Some(form) = &self.integrations.form else {
            return;
        };
        if [&form.name, &form.endpoint, &form.service, &form.account]
            .iter()
        ...
        config.bearer = if service.is_empty() && account.is_empty() {
            None
        } else {
            Some(SecretReference { service, account })
        };
        if let Err(error) = config.validate() {
            self.integrations.error = Some(error.to_string());
            cx.notify();
            return;
        }
    ...
    fn change_mcp(&mut self, edit: McpEdit, cx: &mut Context<Self>) {
        ...
        self.integration_job(
            async move {
                controller
                    .configure_mcp(revision, edit)
                    .await
                    .map(|value| Reply::Changed(value, message))
                    .map_err(|e| e.to_string())
            },
            cx,
        );
        ...
        self.integration_job(
            async move {
                let result = controller
                    .test_mcp(revision, id.clone())
                    .await
                    .map_err(|e| e.to_string());
                Ok(Reply::Probed(id, revision, result))
            },
            cx,
        );
    }
    fn confirm_integration(&mut self, cx: &mut Context<Self>) {
        let Some(confirm) = self.integrations.confirm.clone() else {
            return;
        };
        match confirm {
            ...
            Confirm::Disconnect(task) => {
                ...
                self.integration_job(
                    async move {
                        controller
                            .disconnect_mcp_agent(task)
                            .await
                            .map(|_| Reply::Disconnected)
                            .map_err(|e| e.to_string())
                    },
                    cx,
                );
