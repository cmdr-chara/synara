...
use super::*;
use crate::{WorkspaceError, WorkspaceResult, WorkspaceService, integrations::*};
...
fn change(
    store: &mut Store,
    revision: u64,
    edit: impl FnOnce(&Connection, &mut IntegrationSettings) -> WorkspaceResult<()>,
    ...
    if value.revision != revision {
        return Err(invalid(
            "Integrations changed. Reload and review the current settings before retrying.",
        ));
    }
    edit(&tx, &mut value)?;
    value.validate()?;
...
impl WorkspaceService {
    ...
    pub async fn review_skill(&self, path: std::path::PathBuf) -> WorkspaceResult<SkillReview> {
        tokio::task::spawn_blocking(move || SkillReview::inspect(path))
            .await
    ...
    pub async fn install_skill(
        ...
        let mut skill = tokio::task::spawn_blocking(move || review.recheck())
            .await
            .map_err(|_| WorkspaceError::Worker)??;
        self.access(move |store| {
            change(store, revision, move |_, value| {
                if let Some(id) = replace {
                    ...
                    if skill.previous_origins.len() > 8 {
                        skill.previous_origins.remove(0);
                    }
                    *old = skill;
                } else {
                    if value
                        .skills
                        .iter()
                        .any(|old| old.origin.sha256 == skill.origin.sha256)
                    {
    ...
    pub async fn edit_skill(
        ...
        self.access(move |store| {
            change(store, revision, move |_, value| {
                match edit {
                    SkillEdit::SetEnabled { id, enabled } => {
                        value
                            .skills
                            .iter_mut()
                            .find(|s| s.id == id)
    ...
    pub(crate) async fn edit_mcp(
        &self,
        revision: u64,
        edit: McpEdit,
    ) -> WorkspaceResult<IntegrationSettings> {
        self.access(move |store| {
            change(store, revision, move |tx, value| {
                ...
                if task.is_none()
                    && !matches!(
                        &edit,
                        McpEdit::Remove(_) | McpEdit::SetEnabled { enabled: false, .. }
                    )
                {
