...
async function requestJson(path,body){
  ...
  if(!response.ok){
    const errors={unauthorized:'The token was not accepted.',interaction_expired:'This request is no longer active. Refresh to review current requests.',invalid_interaction_reply:'The response does not match the current request. Check required fields and offered choices.',draft_changed:'The saved draft changed in another tab. Reload and review it before running.',run_active:'This task already has an active web run.',authentication_required:'Sign in with the agent locally before running it here.'};
...
function interactionPanel(base,current,onAnswered){
  ...
  function create(item){
    ...
    function action(label,payload,review=false){
      const control=button(label,async()=>{
        ...
        try{await postJson(base+'/interactions',body);if(current()){feedback.textContent='Response submitted.';await onAnswered()}}
  ...
  return {element,
    update(data){
      ...
      note.textContent=data.more?'More requests are queued. Respond to these to review the next requests.':items.length?'Agent-provided requests. No tool output is shown here. Deny anything you cannot assess. Answers go to the configured agent.':'No pending requests. Connection sign-in and URL requests remain unsupported.';
    },
...
async function showThread(task,before=null){
  ...
  try{
    ...
    function showRun(run){running=run.state==='running'||run.state==='stopping';runStatus.textContent=`Web run: ${run.state}${run.error?` (${run.error})`:''}`;updateButtons()}
    if(unsent.truncated){const clipped=document.createElement('p');clipped.textContent=unsent.text+'…';pending.append(clipped);const note=document.createElement('small');note.textContent='This draft is too large to edit or run in the local browser.';pending.append(note)}
      ...
      runButton=button('Run draft',async()=>{
        ...
        if(!confirm('Run this prompt with the configured local agent in this task folder? The agent can use its existing local permissions. One-time permission requests and task questions require explicit replies in this page. Sign-in links are not supported. The draft is retained for review and retry.'))return;
        const expected_draft=activeDraftSaved;
        posting=true;updateButtons();
        try{
          const result=await postJson(base+'/run',{text,expected_draft});
