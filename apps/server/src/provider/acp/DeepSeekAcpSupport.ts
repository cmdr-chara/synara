...
export function buildDeepSeekHarnessConfig(modelInput: string | undefined): string {
...
const makeGeneratedConfig = Effect.fnUntraced(function* (model: string | undefined) {
  ...
  yield* Effect.tryPromise({
    try: () => fs.writeFile(configPath, buildDeepSeekHarnessConfig(model), "utf8"),
    catch: (cause) =>
      new AcpErrors.AcpTransportError({
...
export const makeDeepSeekAcpRuntime = (
  ...
  Effect.gen(function* () {
    const configPath = input.settings?.configPath?.trim() || (yield* makeGeneratedConfig(input.settings?.model));
