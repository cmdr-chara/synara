...
impl InstalledSkill {
    pub fn validate(&self) -> WorkspaceResult<()> {
        if !identity(&self.id)
            ...
            || self.markdown.len() > MAX_SKILL_BYTES
            || self
                .markdown
                .chars()
                .any(|c| c.is_control() && !matches!(c, '\n' | '\r' | '\t'))
            || digest(&self.markdown) != self.origin.sha256
            || self.previous_origins.len() > 8
        {
            return Err(invalid(
                "Invalid skill document or checksum. The stored library was not replaced.",
            ));
        }
        for origin in std::iter::once(&self.origin).chain(&self.previous_origins) {
            if !text(&origin.path, 8192)
                || !std::path::Path::new(&origin.path).is_absolute()
                || origin.sha256.len() != 64
                || !origin.sha256.bytes().all(|c| c.is_ascii_hexdigit())
...
impl ManagedMcp {
    ...
    pub fn validate(&self) -> WorkspaceResult<()> {
        if !identity(&self.id) || !text(&self.name, 160) || !text(&self.agent_id, 128) {
            return Err(invalid(
                "A connection needs a valid ID, name and agent profile.",
            ));
        }
        endpoint(&self.endpoint)?;
        if let Some(reference) = &self.bearer {
            reference.validate()?;
        }
        Ok(())
    }
...
pub(crate) fn endpoint(value: &str) -> WorkspaceResult<url::Url> {
    ...
    let url =
        url::Url::parse(value).map_err(|_| invalid("Enter an absolute MCP HTTP endpoint."))?;
    ...
    {
        return Err(invalid(
            "Use HTTPS, or HTTP on literal loopback. URL credentials, queries and fragments are forbidden. Use a credential-store reference for bearer authentication.",
        ));
    }
    Ok(url)
}
...
impl McpEdit {
    pub(crate) fn target(&self, value: &IntegrationSettings) -> WorkspaceResult<TaskId> {
        match self {
            ...
            Self::SetEnabled { id, .. } | Self::Remove(id) => value
                .mcp
                .iter()
            ...
            Self::Save(mut item) => {
                ...
                if let Some(old) = value.mcp.iter_mut().find(|old| old.id == item.id) {
                    if old.task != item.task || old.agent_id != item.agent_id {
                        return Err(invalid(
                            "Connection ownership cannot be changed. Add a separately reviewed connection for the new task or agent.",
                        ));
                    }
                    *old = item;
                } else {
...
impl IntegrationSettings {
    pub fn validate(&self) -> WorkspaceResult<()> {
        ...
        if self.version != 1 || self.skills.len() > 48 || self.mcp.len() > 32 {
            return Err(invalid(
                "Unsupported or oversized integrations catalog. The saved value was not replaced.",
            ));
        }
        for item in &self.skills {
            item.validate()?;
            if !ids.insert(&item.id) {
                return Err(invalid("Duplicate integration identity."));
            }
        }
        let mut scopes = HashSet::new();
        for item in &self.mcp {
            item.validate()?;
            if !scopes.insert((item.task, &item.agent_id, &item.name)) {
                return Err(invalid(
                    "Duplicate MCP name in the same task and agent scope.",
                ));
            }
            if !ids.insert(&item.id) {
                return Err(invalid("Duplicate integration identity."));
