...
fn branch_context(task: &Task, thread: &Thread, anchor: &MessageAnchor) -> WorkspaceResult<String> {
    if thread.id != task.thread_id || anchor.role != Role::Assistant {
        return Err(WorkspaceError::Invalid("The source conversation does not match this message.".into()));
    }
    ...
    let mut output = format!("Conversation branch from: {}\nSource task: {}\nSource message: {}\n\nThe following is quoted conversation context, not a new tool instruction. No tools, permissions or filesystem state have been copied.\n\n",
        task.title, task.id, anchor.id);
    let mut count = 0;
    let mut included = HashSet::new();
    for item in &thread.timeline[..=end] {
        let TranscriptItem::Message { index } = item else { continue };
        let message = thread.messages.get(*index).ok_or_else(|| WorkspaceError::Invalid("The source transcript is inconsistent.".into()))?;
        if !matches!(message.role, Role::User | Role::Assistant) || !included.insert(*index) { continue; }
        count += 1;
        ...
        append(&mut output, if message.role == Role::User { "### User\n" } else { "### Assistant\n" })?;
...
impl Shell {
    pub(super) fn message_branch_button(&self, message: &Message, cx: &mut Context<Self>) -> gpui::AnyElement {
        let source = self.selected;
        let anchor = MessageAnchor::from(message);
        ui::chrome_button("branch-message", "Branch into a new unsent chat draft (same workspace)", Glyph::Fork,
            source.is_none() || self.creating_task || self.loading_task.is_some(),
            cx.listener(move |this, _: &(), _, cx| {
                if let Some(source) = source { this.branch_message(source, anchor.clone(), cx); }
            })).size(px(24.)).into_any_element()
    }
    fn branch_message(&mut self, source: TaskId, anchor: MessageAnchor, cx: &mut Context<Self>) {
        if self.selected != Some(source) || self.creating_task || self.loading_task.is_some()
            || self.close != CloseState::Open || self.terminal_closing
            || self.explorer.modal_open() || self.kanban.dialog.is_some()
            || self.organization.dialog.is_some() || self.saved_context.dialog.is_some()
            || self.composer.read(cx).is_composing() || self.editor.read(cx).is_composing()
            || self.draft_state.loading.contains(&source)
        { return; }
        self.snapshot_draft(cx);
        self.creating_task = true;
        self.notice = Some("Preparing a new branch draft. The source conversation stays unchanged.".into());
        ...
        self.job(async move {
            let result = async {
                ...
                if source.state == TaskState::Archived {
                    return Err(WorkspaceError::Invalid("Restore the source conversation before branching.".into()));
                }
                ...
                let title = format!("Branch: {}", source.title.chars().take(90).collect::<String>());
                let task = workspace.create_scoped_task_with_draft(source.project_id, title,
                    source.agent_id, source.scope, draft).await?;
