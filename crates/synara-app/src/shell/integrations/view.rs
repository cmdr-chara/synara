...
fn section(title: &str) -> gpui::Div {
    div().mt_4().mb_2().font_weight(gpui::FontWeight::SEMIBOLD).child(title.to_owned())
}
fn row() -> gpui::Div {div().py_3().border_b_1().border_color(rgb(palette().border)).flex().flex_col().gap_2()}
impl Shell {
    pub(super) fn integration_settings(&self, page: Section, cx: &mut Context<Self>) -> gpui::AnyElement {
        let state=&self.integrations;
        let toolbar=div().flex().gap_2().items_center()
            .child(div().flex_1().relative().child(state.query.clone()).child(ui::layout_probe("integrations-search")))
            .child(ui::button("integrations-reload","Reload",false).relative().child(ui::layout_probe("integrations-reload" )).on_click(cx.listener(|this,_,_,cx|this.load_integrations(cx))));
        let filters=div().flex().gap_2().children([(None,"All"),(Some(true),"Enabled"),(Some(false),"Disabled")].into_iter().enumerate().map(|(i,(filter,label))|
            ui::button(("integration-filter",i),label,state.filter==filter).on_click(cx.listener(move |this,_,_,cx|{this.integrations.filter=filter;cx.notify();}))));
        let mut body=div().flex().flex_col().gap_2().child(toolbar).child(filters)
            .children(state.error.as_ref().map(|error|div().text_color(rgb(palette().error)).child(error.clone())))
            .children(state.notice.as_ref().map(|text|note(text.clone())))
            .children(state.busy.then(||note("Working on the requested operation... No other integration action is queued.")));
        if let Some(confirm)=&state.confirm {
            let message=match confirm {
                Confirm::Skill(_)=>"Remove this document from Synara's library? The original file, provider skills and previously inserted drafts will not be changed.",
                Confirm::Mcp(_)=>"Remove this task-scoped connection? Synara will stop sharing it with new sessions after the old session is retired. Revoke the external token separately at its provider. The credential-store entry is not deleted.",
                Confirm::Disconnect(_)=>"Disconnect this agent process? Other tasks may share it. Active prompts must finish first. No tasks will restart automatically and no provider credential will be revoked.",
            };
            body=body.child(row().child(note(message)).child(div().flex().gap_2()
                .child(ui::button("integration-confirm","Confirm",false).relative().child(ui::layout_probe("integration-confirm" )).on_click(cx.listener(|this,_,_,cx|this.confirm_integration(cx))))
                .child(ui::button("integration-cancel-confirm","Cancel",false).relative().child(ui::layout_probe("integration-cancel-confirm" )).on_click(cx.listener(|this,_,_,cx|{if !this.integrations.busy{this.integrations.confirm=None;}cx.notify();})))));
        }
        let Some(value)=&state.value else {return body.child(note("Integration settings are not loaded. Reload to read them. No repository configuration is imported automatically.")).into_any_element();};
        match page {
            Section::Plugins=>{
                ...
                for (name,description,target) in [
                    ("MCP connections","Explicit HTTP discovery, task/agent consent and credential references.",Section::Mcp),
                    ("Skill documents","Reviewed Markdown imports with origin receipts. Explicit draft insertion only.",Section::Skills),
                ] {
                    if state.matches(true,&[name,description],cx) {
                        body=body.child(row().child(div().flex().items_center().justify_between().child(name)
                            .child(ui::button(SharedString::from(format!("manage-{name}")),"Manage",false).on_click(cx.listener(move |this,_,_,cx|this.open_settings_section(target,cx)))))
                ...
                for item in &value.mcp {
                    if state.matches(item.enabled,&[&item.name,&item.endpoint,&item.agent_id],cx) {
                        count+=1;
                        body=body.child(row().child(item.name.clone()).child(note(format!("MCP configuration | {} | Task {} | Agent {}",if item.enabled{"Enabled for future sessions"}else{"Disabled"},item.task,item.agent_id))));
                    }
                }
                for item in &value.skills {
                    if state.matches(item.enabled,&[&item.title,&item.description,&item.origin.path],cx) {
                        count+=1;
                        body=body.child(row().child(item.title.clone()).child(note(format!("Skill document | Installed in Synara | {}",if item.enabled{"Draft insertion enabled"}else{"Disabled"}))));
                    }
                }
                if count==0 {body=body.child(note("No managed items match this filter."));}
                ...
                } else {body=body.child(note("No current negotiation evidence. Opening this page does not connect an agent."));}
            },
            Section::Skills=>{
                ...
                if let Some(preview)=&state.preview {
                    body=body.child(self.skill_preview(preview))
                        .child(ui::button("skill-close-preview","Close preview",false).relative().child(ui::layout_probe("skill-close-preview" )).on_click(cx.listener(|this,_,_,cx|{this.integrations.preview=None;cx.notify();})));
                }
                body=body.child(section("Installed documents"));
                let mut count=0;
                for item in &value.skills {
                    if !state.matches(item.enabled,&[&item.title,&item.description,&item.origin.path],cx) {continue;}
                    count+=1;
                    let preview=item.clone();let toggle=item.id.clone();let remove=item.id.clone();let update=item.id.clone();let insert=item.id.clone();let enabled=item.enabled;
                    let mut actions=div().flex().flex_wrap().gap_2()
                        .child(ui::button(("skill-preview",count),"Review",false).relative().child(ui::layout_probe_slot("skill-preview",count)).on_click(cx.listener(move |this,_,_,cx|{this.integrations.preview=Some(preview.clone());cx.notify();})))
                        .child(ui::button(("skill-toggle",count),if enabled{"Disable"}else{"Enable for drafts"},false).relative().child(ui::layout_probe_slot("skill-toggle",count)).on_click(cx.listener(move |this,_,_,cx|this.change_skill(SkillEdit::SetEnabled{id:toggle.clone(),enabled:!enabled},cx))))
                        .child(ui::button(("skill-update",count),"Review update",false).relative().child(ui::layout_probe_slot("skill-update",count)).on_click(cx.listener(move |this,_,_,cx|this.choose_skill(Some(update.clone()),cx))))
                        .child(ui::button(("skill-remove",count),"Remove",false).relative().child(ui::layout_probe_slot("skill-remove",count)).on_click(cx.listener(move |this,_,_,cx|{if !this.integrations.busy{this.integrations.confirm=Some(Confirm::Skill(remove.clone()));}cx.notify();})));
                    if enabled && self.selected.is_some() {actions=actions.child(ui::button(("skill-insert",count),"Insert into draft",false).relative().child(ui::layout_probe_slot("skill-insert",count)).on_click(cx.listener(move |this,_,_,cx|this.insert_skill(insert.clone(),cx))));}
                    body=body.child(row().child(item.title.clone()).child(note(item.description.clone()))
                        .child(note(format!("Synara-owned | {} | Version {} | {}",if enabled{"Enabled for drafts"}else{"Disabled"},item.origin.version.as_deref().unwrap_or("not declared"),item.origin.path)))
                        .child(note(format!("SHA-256 {}",item.origin.sha256))).child(actions));
                }
                if count==0 {body=body.child(note("No installed skill documents match this filter."));}
            ...
            Section::Mcp=>{
                ...
                let mut count=0;
                for item in &value.mcp {
                    if !state.matches(item.enabled,&[&item.name,&item.endpoint,&item.agent_id],cx) {continue;}
                    count+=1;
                    let edit=item.clone();let toggle=item.id.clone();let remove=item.id.clone();let test=item.id.clone();let enabled=item.enabled;let task=item.task;
                    let known_unsupported=self.selected==Some(item.task) && self.task().is_some_and(|task|task.agent_id==item.agent_id)
                        && self.details.as_ref().is_some_and(|d|!d.connection.capabilities.mcp_http);
                    let mut actions=div().flex().flex_wrap().gap_2()
                        .child(ui::button(("mcp-edit",count),"Edit",false).relative().child(ui::layout_probe_slot("mcp-edit",count)).on_click(cx.listener(move |this,_,_,cx|this.edit_mcp_form(Some(edit.clone()),cx))))
                        .child(ui::button(("mcp-test",count),"Test and discover",false).relative().child(ui::layout_probe_slot("mcp-test",count)).on_click(cx.listener(move |this,_,_,cx|this.test_mcp_row(test.clone(),cx))))
                        .child(ui::button(("mcp-remove",count),"Remove",false).relative().child(ui::layout_probe_slot("mcp-remove",count)).on_click(cx.listener(move |this,_,_,cx|{if !this.integrations.busy{this.integrations.confirm=Some(Confirm::Mcp(remove.clone()));}cx.notify();})))
                        .child(ui::button(("mcp-disconnect",count),"Disconnect agent...",false).relative().child(ui::layout_probe_slot("mcp-disconnect",count)).on_click(cx.listener(move |this,_,_,cx|{if !this.integrations.busy{this.integrations.confirm=Some(Confirm::Disconnect(task));}cx.notify();})));
                    if enabled || !known_unsupported {actions=actions.child(ui::button(("mcp-enable",count),if enabled{"Disable"}else{"Enable for this scope"},false).relative().child(ui::layout_probe_slot("mcp-enable",count)).on_click(cx.listener(move |this,_,_,cx|this.change_mcp(McpEdit::SetEnabled{id:toggle.clone(),enabled:!enabled},cx))));}
                    ...
                    } else {entry=entry.child(note("Not tested in this view. Saving and enabling do not prove a connection."));}
                    body=body.child(entry);
                }
                if count==0 {body=body.child(note("No configured MCP connections match this filter."));}
