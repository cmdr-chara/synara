...
use crate::{ProviderSettings, Store, WorkspaceError, WorkspaceResult, WorkspaceService, now_ms};
use serde::{Deserialize, Serialize};
use sha2::{Digest, Sha256};
use std::path::PathBuf;
use synara_core::{ProjectId, Role, TaskId, TaskState, ThreadId};
...
pub(crate) fn recap_key(id: TaskId) -> String {
...
impl ThreadRecap {
    pub(crate) fn validate(&self) -> WorkspaceResult<()> {
        if self.version != 1
            || self.revision == 0
            || !bounded(&self.text, MAX_RECAP_BYTES)
            || !valid_hash(&self.source.sha256)
            || !valid_hash(&self.profile_sha256)
            || self.source.included_messages == 0
            || self.source.included_messages > MAX_MESSAGES
            || self.source.omitted_messages > 200_000
            || self.generated_at_ms < 0
            || !bounded(&self.provider_id, 256)
            || !bounded(&self.model_id, 512)
            || !bounded(&self.endpoint, 2048)
            || serde_json::to_vec(self)
                .map_err(|_| invalid("Invalid recap."))?
                .len()
                > MAX_CACHE_BYTES
        {
            return Err(invalid(
                "Unsupported, invalid or oversized recap cache. It was not overwritten.",
            ));
        }
        Ok(())
    }
...
impl RecapReview {
    pub fn expected_revision(&self) -> u64 {
        self.cached.as_ref().map_or(0, |c| c.revision)
    }
}
pub(crate) fn snapshot(store: &Store, id: TaskId) -> WorkspaceResult<RecapSnapshot> {
    ...
    for message in visible.iter().rev() {
        if chosen.len() >= MAX_MESSAGES {
            break;
        }
        if message.text.len() > MAX_SOURCE_BYTES || message.id.len() > 512 {
            continue;
        }
...
pub(crate) fn cached(store: &Store, id: TaskId) -> WorkspaceResult<Option<ThreadRecap>> {
    let Some(raw) = store.preference_raw(&recap_key(id))? else {
        return Ok(None);
    };
    if raw.len() > MAX_CACHE_BYTES {
        return Err(invalid("Oversized recap cache. It was not overwritten."));
    }
    let result: ThreadRecap = serde_json::from_str(&raw)
        .map_err(|_| invalid("Unreadable recap cache. It was not overwritten."))?;
    result.validate()?;
    if result.source.task != id {
        return Err(invalid("Recap cache belongs to another task."));
    }
    Ok(Some(result))
}
pub(crate) fn assert_current(
    store: &Store,
    reviewed: &RecapSnapshot,
    expected: u64,
) -> WorkspaceResult<()> {
    ...
    if matches!(
        ...
        return Err(invalid(
            "Finish the active turn or pending interaction before generating a recap.",
        ));
    }
    if reviewed.text.is_empty() || snapshot(store, id)? != *reviewed {
        return Err(invalid(
            "Conversation or task ownership changed. Reload and review the source again.",
        ));
    }
    if cached(store, id)?.as_ref().map_or(0, |c| c.revision) != expected {
        return Err(invalid(
            "Another recap was saved. Reload before regenerating.",
        ));
    }
    Ok(())
}
impl WorkspaceService {
    pub async fn review_thread_recap(&self, id: TaskId) -> WorkspaceResult<RecapReview> {
        ...
        self.access(move |store| {
            Ok(RecapReview {
                snapshot: snapshot(store, id)?,
                cached: cached(store, id)?,
                settings,
            })
        })
        .await
    }
    pub(crate) async fn check_recap_source(
        &self,
        reviewed: RecapSnapshot,
        expected: u64,
    ) -> WorkspaceResult<()> {
        self.access(move |store| assert_current(store, &reviewed, expected))
            .await
    }
    pub(crate) async fn finish_thread_recap(
        ...
        reviewed: RecapSnapshot,
        expected: u64,
        mut result: ThreadRecap,
        cancel: CancellationToken,
    ) -> WorkspaceResult<ThreadRecap> {
        self.access(move |store| {
            if cancel.is_cancelled() {
                return Err(invalid("Recap cancelled. The previous cache is preserved."));
            }
            assert_current(store, &reviewed, expected)?;
            result.source = reviewed.source;
            result.revision = expected
                .checked_add(1)
                .ok_or_else(|| invalid("Recap revision exhausted."))?;
...
mod tests {
    ...
    async fn seed(s: &WorkspaceService, root: &std::path::Path) -> TaskId {
        ...
        let t = s
            .create_task(p.id, "Recap test".into(), agent)
            .await
            .unwrap();
        s.record(
            t.thread_id,
            ThreadEvent::TextDelta {
                ...
                role: Role::User,
                text: "Investigate the timeout".into(),
            },
        )
        .await
        .unwrap();
    ...
    fn result(source: RecapSource) -> ThreadRecap {
        ThreadRecap {
            ...
            revision: 1,
            source,
            text: "Context: investigate the timeout.".into(),
    ...
    async fn source_review_and_save_leave_transcript_inert_and_survive_restart() {
        ...
        let saved = s
            .finish_thread_recap(
                r.snapshot.clone(),
                0,
                result(r.snapshot.source.clone()),
                CancellationToken::new(),
            )
            .await
            .unwrap();
        assert_eq!(
            s.review_thread_recap(id).await.unwrap().snapshot,
            r.snapshot
        );
        drop(s);
    ...
    async fn changed_source_and_forged_task_identity_are_rejected() {
        ...
        s.record(
            r.snapshot.source.thread,
            ThreadEvent::TextDelta {
                message_id: Some("user-1".into()),
                role: Role::User,
                text: " with changed requirements".into(),
            },
        )
        .await
        .unwrap();
    ...
    async fn bounds_exclude_hidden_roles_and_report_omitted_whole_messages() {
        ...
        s.record(
            thread,
            ThreadEvent::TextDelta {
                message_id: Some("overlarge".into()),
                role: Role::Assistant,
        ...
        s.record(
            ...
            ThreadEvent::TextDelta {
                ...
                role: Role::Reasoning,
                text: "Hidden reasoning must not be shared".into(),
            },
        )
        .await
        .unwrap();
    ...
    fn malformed_cached_output_and_provenance_fail_closed() {
        ...
        r.text = "x".repeat(MAX_RECAP_BYTES + 1);
        assert!(r.validate().is_err());
        r.text = "safe".into();
        r.profile_sha256 = "fake".into();
        assert!(r.validate().is_err());
    }
    #[tokio::test]
    async fn forged_preview_and_oversized_output_do_not_create_cache() {
        ...
        let r = s.review_thread_recap(id).await.unwrap();
        let mut forged = r.snapshot.clone();
        forged.text = "Unreviewed text".into();
        assert!(s.check_recap_source(forged, 0).await.is_err());
        let mut big = result(r.snapshot.source.clone());
        big.text = "x".repeat(MAX_RECAP_BYTES + 1);
        assert!(
            s.finish_thread_recap(r.snapshot, 0, big, CancellationToken::new())
                .await
                .is_err()
        );
        assert!(s.review_thread_recap(id).await.unwrap().cached.is_none());
