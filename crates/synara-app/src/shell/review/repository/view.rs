...
impl RepositoryPanel {
    fn action(
        ...
        ui::action(
            ...
            None,
            false,
            cx.listener(move |this, _: &(), _, cx| this.open(action.clone(), cx)),
        )
        .text_size(px(12.))
        .relative()
        .child(ui::layout_probe_slot(id.0, id.1))
        .when(self.busy, |el| el.opacity(0.45))
        .into_any_element()
    }
    ...
    fn list(&self, cx: &mut Context<Self>) -> gpui::AnyElement {
        ...
        match self.view {
                    ...
                    rows.push(
                        self.row(
                            ...
                            format!(
                                ...
                                branch.object.chars().take(12).collect::<String>()
                            ),
                        )
                        .child(actions)
                        .into_any_element(),
            ...
            View::Worktrees => {
                for (i, item) in self
                    ...
                    .enumerate()
                    .filter(|(_, w)| {
                        format!("{} {}", w.path.display(), w.branch)
                            .to_lowercase()
                            .contains(&query)
                    })
                    .take(400)
                {
                    ...
                    let copy = path.clone();
                    let mut actions = div().flex().flex_wrap().gap_1().child(
                        ui::action(
                            ("repo-copy-worktree", i),
                            "Copy path",
                            Some(Glyph::Copy),
                            false,
                            move |_: &(), _, cx| {
                        ...
                        && item.path != *self.target.root()
                    {
                        actions = actions.child(self.action(
                            ...
                            "Remove",
                            Action::RemoveWorktree(item.path.clone()),
                            cx,
                        ));
                    }
                    rows.push(
            ...
            View::Stashes => {
                for (i, item) in self
                    ...
                    .stashes
                    .iter()
                    .filter(|s| s.subject.to_lowercase().contains(&query))
                    .take(400)
                    .enumerate()
                {
                    let copy = item.object.clone();
                    rows.push(
                        self.row(item.subject.clone(), item.object.chars().take(12).collect())
                            .child(
                                div()
                                    .flex()
                                    .flex_wrap()
                                    ...
                                    .child(self.action(
                                        ("repo-apply-stash", i),
                                        "Apply and keep",
                                        Action::ApplyStash(item.object.clone()),
                                        cx,
                                    ))
                                    .child(
                                        ui::action(
        ...
        div()
            ...
            .flex_col()
            .children(rows)
            .children((empty && !self.busy).then(|| {
                div()
                    .p_4()
                    .text_size(px(12.))
                    .text_color(rgb(palette().muted))
                    .child(if self.error.is_some() {
                        "The repository could not be fully loaded. Refresh to retry."
                    } else {
                        "No matching items. Create one or change the filter."
                    })
            }))
            .into_any_element()
    }
...
impl Render for RepositoryPanel {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let total = match self.view {
            View::Branches => self.catalog.branches.len(),
            View::Remotes => self.catalog.remotes.len(),
            View::Worktrees => self.catalog.worktrees.len(),
            View::Stashes => self.catalog.stashes.len(),
        };
        let add = match self.view {
            View::Branches => Action::CreateBranch,
            View::Remotes => Action::AddRemote,
            View::Worktrees => Action::AddWorktree,
            View::Stashes => Action::SaveStash,
        };
        div()
            .id("repository-panel")
            .track_focus(&self.focus)
            .relative()
            .child(ui::layout_probe("repository-panel"))
            .flex()
            .flex_col()
            .flex_1()
            .min_h_0()
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .child(format!(
                                "{} · {}",
                                if matches!(self.target, WorkspaceTarget::Local { .. }) {
                                    "Local"
                                } else {
                                    "SSH"
                                },
            ...
            .child(
                div().px_2().flex().flex_wrap().gap_1().children(
                    ...
                    .map(|(index, view)| {
                        ui::action(
                            ...
                            cx.listener(move |this, _: &(), _, cx| {
                                if this.form.is_none() {
                                    this.view = view;
                                    this.query.update(cx, |q, cx| q.clear(cx));
                                    cx.notify();
                                }
                            }),
                        )
                        .text_size(px(12.))
                        .relative()
                        .child(ui::layout_probe_slot("repository-view", index))
                    }),
                ),
            )
            ...
            .children(self.busy.then(|| {
                div()
                    ...
                    .py_2()
                    .text_size(px(12.))
                    .child("Git is running on the selected host. No automatic retries.")
            }))
            ...
            .child(
                div()
                    ...
                    .text_color(rgb(palette().muted))
                    .child(format!(
                        "{total} items · Showing up to 400 filtered results. No force operations."
                    )),
            )
    }
}
