...
impl TextBuffer {
    ...
    pub fn backspace(&mut self) -> Result<(), TextError> {
        ...
        if range.is_empty() {
            range.start = self.text[..range.start]
                .char_indices()
                .last()
                .map_or(0, |(i, _)| i);
        }
    ...
    pub fn move_left(&mut self) {
        let index = if self.selection.is_empty() {
    ...
    pub fn move_right(&mut self) {
        ...
        if self.selection.is_empty() {
            index += self.text[index..].chars().next().map_or(0, char::len_utf8);
        }
        self.selection = index..index;
        self.marked = None;
    }
    pub fn byte_to_utf16(&self, offset: usize) -> Result<usize, TextError> {
        ...
        Ok(self.text[..offset].encode_utf16().count())
    }
    ...
    pub fn mark(&mut self, range: Range<usize>) -> Result<(), TextError> {
        self.validate(&range)?;
        self.marked = Some(range);
        Ok(())
    }
...
mod tests {
    use super::*;
    #[test]
    fn unicode_editing_never_splits_utf8() {
    ...
    #[test]
    fn utf16_rejects_half_surrogates() {
        let b = TextBuffer::new("a😀z".into());
        assert_eq!(b.utf16_to_byte(2), Err(TextError::InvalidRange));
        assert_eq!(b.utf16_to_byte(3), Ok(5));
        assert_eq!(b.byte_to_utf16(5), Ok(3));
    }
    #[test]
    fn selection_replacement_and_marking() {
        let mut b = TextBuffer::new("hello".into());
        b.select(1..4).unwrap();
        b.insert("i").unwrap();
        assert_eq!(b.text(), "hio");
        b.mark(1..2).unwrap();
        b.insert("!").unwrap();
