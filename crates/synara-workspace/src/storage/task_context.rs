...
impl ChecklistItem {
    pub fn new(text: String) -> Self {
        Self { id: uuid::Uuid::new_v4().to_string(), text, done: false }
    }
}
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
impl Default for TaskContext {
    fn default() -> Self {
        Self { version: 1, revision: 0, notes: String::new(), checklist: Vec::new() }
    }
}
impl TaskContext {
    pub fn validate(&self) -> WorkspaceResult<()> {
        let ids = self.checklist.iter().map(|item| &item.id).collect::<HashSet<_>>();
        if self.version != 1 || self.notes.len() > MAX_NOTE_BYTES || self.notes.contains('\0')
            || self.checklist.len() > MAX_CHECKLIST_ITEMS || ids.len() != self.checklist.len()
            || self.checklist.iter().any(|item| uuid::Uuid::parse_str(&item.id).is_err()
                || item.text.trim().is_empty() || item.text.len() > MAX_CHECKLIST_TEXT
                || item.text.chars().any(char::is_control))
        { return Err(WorkspaceError::Invalid("Notes or checklist are invalid or exceed their size limits.".into())); }
        Ok(())
    }
...
fn context_key(task: TaskId) -> String { format!("task-context:{task}") }
fn read_context(connection: &Connection, task: TaskId) -> WorkspaceResult<TaskContext> {
    ...
    let raw: Option<String> = connection.query_row("SELECT data FROM preferences WHERE key=?1", [context_key(task)], |row| row.get(0)).optional().map_err(StorageError::from)?;
...
impl WorkspaceService {
    ...
    pub async fn save_task_context(&self, task: TaskId, expected_revision: u64, mut value: TaskContext) -> WorkspaceResult<TaskContext> {
        value.validate()?;
        self.access(move |store| {
...
mod tests {
    ...
    async fn seed(service: &WorkspaceService, path: &Path) -> Task {
        ...
        service.create_task(project.id, "Notes".into(), agent).await.unwrap()
    }
    #[tokio::test]
    async fn notes_and_ordered_checklist_survive_restart_without_sending_or_changing_drafts() {
        ...
        service.save_task_draft(task.id, "Unsent prompt".into()).await.unwrap();
        let mut value = TaskContext { notes: "Caffè 日本語\nKeep exact spacing  ".into(), ..Default::default() };
        value.checklist.push(ChecklistItem::new("Review UI".into()));
        let mut done = ChecklistItem::new("Read source".into()); done.done = true;
        value.checklist.insert(0, done);
        let value = service.save_task_context(task.id, 0, value).await.unwrap();
        assert_eq!(value.revision, 1);
        assert!(value.as_prompt_context().contains("- [x] Read source\n- [ ] Review UI"));
        drop(service);
        ...
        assert!(reopened.thread(task.thread_id).await.unwrap().messages.is_empty());
    ...
    async fn stale_invalid_and_future_edits_preserve_saved_notes() {
        ...
        let saved = a.save_task_context(task.id, 0, TaskContext { notes: "Keep".into(), ..stale.clone() }).await.unwrap();
        assert!(b.save_task_context(task.id, 0, TaskContext { notes: "Stale overwrite".into(), ..stale }).await.is_err());
        assert!(a.save_task_context(task.id, 1, TaskContext { notes: "x".repeat(MAX_NOTE_BYTES+1), ..saved.clone() }).await.is_err());
        assert_eq!(b.task_context(task.id).await.unwrap(), saved);
        a.access(move |store| { store.set_preference(&context_key(task.id), &serde_json::json!({"version":99}))?; Ok(()) }).await.unwrap();
        assert!(a.task_context(task.id).await.is_err());
        assert!(a.save_task_context(task.id, 0, TaskContext::default()).await.is_err());
        a.access(move |store| { assert_eq!(store.preference::<serde_json::Value>(&context_key(task.id))?.unwrap()["version"], 99); Ok(()) }).await.unwrap();
    }
    #[tokio::test]
    async fn context_is_task_scoped_and_is_cleaned_up_by_permanent_deletion() {
        ...
        service.save_task_context(task.id, 0, TaskContext { notes: "Only this chat".into(), ..Default::default() }).await.unwrap();
        assert_eq!(service.task_context(other.id).await.unwrap(), TaskContext::default());
        ...
        assert!(service.save_task_context(task.id, 0, TaskContext::default()).await.is_err());
        service.access(move |store| { assert!(store.preference_raw(&context_key(task.id))?.is_none()); Ok(()) }).await.unwrap();
    }
}
