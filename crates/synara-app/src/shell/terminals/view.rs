...
impl Shell {
    fn move_terminal_tab(
        ...
        let Some(index) = group.layout.tabs.iter().position(|tab| tab.id == id) else {
            return;
        };
        let next = if backwards {
            ...
            index
                .checked_add(1)
                .filter(|n| *n < group.layout.tabs.len())
        };
    ...
    fn terminal_action(
        ...
        if self.terminal_entry(key).is_none()
            || self.terminal_closing
            || self
                .terminals
                .groups
                .get(&key.scope)
                .is_some_and(|group| group.loading)
        {
            return;
        }
        match action {
            Action::Start => {
                self.request_terminal_action(key.scope.clone(), key.id, StopAction::Restart, cx)
            }
            Action::Stop => {
                self.request_terminal_action(key.scope.clone(), key.id, StopAction::Keep, cx)
            }
            Action::Close => {
                self.request_terminal_action(key.scope.clone(), key.id, StopAction::Close, cx)
            }
            Action::Copy => {
                let text = self
                    .terminal_entry(key)
            ...
            Action::Find => {
                ...
                let focus = if entry.search {
                    entry.query.read(cx).focus_handle(cx)
                } else {
                    entry.view.read(cx).focus_handle(cx)
                };
            ...
            Action::Bottom => self
                ...
                .unwrap()
                .view
                .update(cx, |view, cx| view.follow_output(cx)),
        }
    ...
    fn terminal_pane(
        ...
        let mut pane = div()
            ...
            .child(
                div()
                    ...
                    .child(
                        ui::action(
                            ("terminal-focus", id as usize),
                            tab.name.clone(),
                            Some(Glyph::Terminal),
                            group.layout.active == Some(id),
                            cx.listener(move |this, _: &(), window, cx| {
                                this.select_terminal(&select_scope, id, window, cx)
                            }),
                        )
                        .flex_1()
                        .min_w_0()
                ...
                div()
                    ...
                    .children(
                        [
                            ...
                            (
                                "interrupt-shell",
                                "Interrupt this shell",
                                Glyph::Stop,
                                Action::Interrupt,
                                busy || entry.process.is_none() || entry.exited,
                            ),
                            (
                                "stop-shell",
                                "Stop this shell",
                            ...
                            (
                                "terminal-copy",
                                "Copy selection or visible rows",
                                Glyph::Copy,
                                Action::Copy,
                                busy,
                            ),
                            (
                                "terminal-to-chat",
                                "Add selection or visible rows to unsent draft",
                                Glyph::Chat,
                                Action::Draft,
                                busy || self.selected.is_none(),
                            ),
                            (
                                "terminal-bottom",
                                "Follow live output",
                        ...
                        .into_iter()
                        .map(
                            |(control, label, glyph, action, disabled)| {
                                let key = key.clone();
                                ui::chrome_button(
                                    control,
                                    label,
                                    ...
                                    disabled,
                                    cx.listener(move |this, _: &(), window, cx| {
                                        this.terminal_action(&key, action, window, cx)
                                    }),
                                )
                                .size(px(26.))
                            },
                ...
                div()
                    ...
                    .child(ui::action(
                        ...
                        cx.listener(move |this, _: &(), _, cx| {
                            this.rename_terminal(&save_scope, id, true, cx)
                        }),
                    ))
                    .child(ui::action(
                        "terminal-cancel-name",
                        "Cancel",
                        None,
                        false,
                        cx.listener(move |this, _: &(), _, cx| {
                            this.rename_terminal(&cancel_scope, id, false, cx)
                        }),
                    )),
            );
        }
        if entry.search {
            let previous_scope = scope.clone();
                ...
                .child(
                    div()
                        ...
                        .child(
                            div()
                                .text_size(px(11.))
                                .child(format!("{}/{}", entry.matches.0, entry.matches.1)),
                        )
                        .child(ui::chrome_button(
                            "terminal-find-prev",
                            "Previous visible match",
                            Glyph::Back,
                            false,
                            cx.listener(move |this, _: &(), _, cx| {
                                this.search_terminal(&previous_scope, id, Some(true), cx)
                            }),
                        ))
                        .child(ui::chrome_button(
                            "terminal-find-next",
                            ...
                            false,
                            cx.listener(move |this, _: &(), _, cx| {
                                this.search_terminal(&next_scope, id, Some(false), cx)
                            }),
                        )),
                )
                .child(
                    div()
                        ...
                        .text_size(px(11.))
                        .text_color(rgb(palette().muted))
                        .child("Visible rows only. Scroll for earlier output."),
                );
        }
        if let Some((confirmed_id, action)) = group
            .confirmation
            .filter(|(confirmed_id, _)| *confirmed_id == id)
        {
            let accept_scope = scope.clone();
            let cancel_scope = scope.clone();
            let label = match action {
                StopAction::Keep => "Stop shell",
                StopAction::Close => "Stop and close",
                StopAction::Restart => "Stop and restart",
            };
        ...
        if let Some(title) = metadata.title().filter(|title| !title.is_empty()) {
            pane = pane.child(
                div()
                    ...
                    .min_w_0()
                    .text_ellipsis()
                    .child(title.chars().take(160).collect::<String>()),
            );
        }
            ...
            .filter(|directory| !directory.is_empty())
        {
            pane = pane.child(
                div()
                    ...
                    .text_ellipsis()
                    .child(format!(
                        "Shell directory hint: {}",
                        directory.chars().take(160).collect::<String>()
                    )),
            );
        }
        ...
        if let Some(error) = entry
            .error
            .as_deref()
            .or(entry.view.read(cx).terminal_error())
        {
            pane = pane.child(
                div()
                    .px_2()
        ...
        pane = pane.child(if busy {
            div()
                ...
                .p_3()
                .text_size(px(12.))
                .child(if entry.starting {
                    "Starting this shell..."
                } else {
                    "Waiting for this shell to stop..."
                })
                .into_any_element()
            ...
            div()
                ...
                .child(
                    div()
                        .text_size(px(12.))
                        .child("This tab is stopped. Restoring tabs never runs commands."),
                )
                .child(ui::action(
                    "terminal-start-empty",
                    "Start shell",
                    Some(Glyph::Terminal),
                    false,
                    cx.listener(move |this, _: &(), window, cx| {
                        this.terminal_action(&start, Action::Start, window, cx)
                    }),
                ))
                .into_any_element()
        } else {
            div()
                .id("terminal-screen")
                .relative()
                .child(ui::layout_probe("terminal-screen"))
                .flex_1()
                .min_h_0()
                .min_w_0()
                .overflow_hidden()
        ...
        if let Some(code) = entry.view.read(cx).exit_code() {
            pane = pane.child(
                div()
                    ...
                    .text_color(rgb(palette().muted))
                    .child(format!(
                        "Exit status {code}. Output retained until this tab is closed or restarted."
                    )),
            );
        }
        pane.into_any_element()
    }
    pub(in crate::shell) fn terminal_panel(
        &self,
        width: f32,
        cx: &mut Context<Self>,
    ) -> gpui::AnyElement {
        let Some(scope) = self.terminal_scope() else {
            return div()
                .p_3()
                .child("Open a project to use terminals.")
                .into_any_element();
        };
        ...
        if group.loading || !group.loaded {
            ...
            return div()
                ...
                .child(if group.loading {
                    "Restoring terminal tabs. No shell is being started."
                } else {
                    "Terminal layout could not be loaded. The saved value was preserved."
                })
                .children(
                    group
                        .load_error
                ...
                .child(ui::action(
                    "terminal-reload-initial",
                    "Retry",
                    None,
                    false,
                    cx.listener(move |this, _: &(), _, cx| {
                        this.load_terminals(retry_scope.clone(), cx)
                    }),
                ))
                .into_any_element();
        }
        let mut root = div()
            ...
            .relative()
            .child(ui::layout_probe("terminal-workspace"))
            .flex_1()
            .min_w_0()
            .min_h_0()
            .flex()
            ...
            .child(
                div()
                    ...
                    .children(group.layout.active.map(|id| {
                        let left = scope.clone();
                        let right = scope.clone();
                        div()
                            .flex()
                            .items_center()
                            .child(
                                ui::chrome_button(
                                    ...
                                    false,
                                    cx.listener(move |this, _: &(), _, cx| {
                                        this.move_terminal_tab(&left, id, true, cx)
                                    }),
                                )
                                .size(px(24.)),
                            )
                            .child(
                                ui::chrome_button(
                                    ...
                                    false,
                                    cx.listener(move |this, _: &(), _, cx| {
                                        this.move_terminal_tab(&right, id, false, cx)
                                    }),
                                )
                                .size(px(24.)),
                            )
                    ...
                    .children(group.layout.secondary.map(|_| {
                        ui::action(
                            "terminal-axis",
                            ...
                            cx.listener(|this, _: &(), _, cx| {
                                if let Some(scope) = this.terminal_scope()
                                    && let Some(group) = this.terminals.groups.get_mut(&scope)
                                {
                    ...
                    .children(group.layout.secondary.map(|_| {
                        div().flex().items_center().gap_1().children(
                            [
                                (
                                    "terminal-shrink",
                                    "Less room for first pane",
                                    Glyph::Minimize,
                                    -5i16,
                                ),
                                (
                                    "terminal-balance",
                                    "Balance terminal panes",
                                    Glyph::Restore,
                                    0i16,
                                ),
                                (
                                    "terminal-grow",
                                    "More room for first pane",
                                    Glyph::Plus,
                                    5i16,
                                ),
                            ]
                            .into_iter()
                            .map(|(id, label, glyph, amount)| {
                                ui::chrome_button(
                                    id,
                                    label,
                                    glyph,
                                    ...
                                    cx.listener(move |this, _: &(), _, cx| {
                                        if let Some(scope) = this.terminal_scope()
                                            && let Some(group) =
                                                this.terminals.groups.get_mut(&scope)
                                        {
                                            group.layout.primary_percent = if amount == 0 {
                                                50
                                            } else {
                                                (i16::from(group.layout.primary_percent) + amount)
                                                    .clamp(25, 75)
                                                    as u8
                                            };
                                            group.changed();
            ...
            .child(
                div()
                    ...
                    .child(format!(
                        "{} · {}",
                        if matches!(group.target, WorkspaceTarget::Ssh { .. }) {
                            "SSH"
                        } else {
                            "Local"
                        },
            ...
            .child(
                div()
                    ...
                    .border_b_1()
                    .border_color(rgb(palette().border))
                    .children(group.layout.tabs.iter().map(|tab| {
                        let select_scope = scope.clone();
                        let id = tab.id;
                        let entry = &group.entries[&id];
                        let running = entry.starting || entry.process.is_some() && !entry.exited;
                        ui::action(
                            ("terminal-tab", id as usize),
                            format!("{}{}", tab.name, if running { " •" } else { "" }),
                            Some(Glyph::Terminal),
                            group.layout.active == Some(id),
                            cx.listener(move |this, _: &(), window, cx| {
                                this.select_terminal(&select_scope, id, window, cx)
        ...
        if let Some(primary) = group.layout.active {
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .when(!stacked, |el| el.w(gpui::relative(ratio)).h_full())
                            .child(pane),
                    )
                    .child(
                        div()
                            ...
                            .min_h_0()
                            .min_w_0()
                            .when(stacked, |el| el.pt_1())
                            .when(!stacked, |el| el.pl_1())
                            .child(self.terminal_pane(&scope, secondary, cx)),
                    )
                    .into_any_element()
            } else {
                div()
            ...
            root = root.child(div().flex_1().p_3().child(
                "No terminal tabs. New stopped tab creates a workspace without executing commands.",
            ));
        }
        root.child(
            div()
                .text_size(px(11.))
                .text_color(rgb(palette().muted))
                .child(if group.saving {
                    "Saving tab layout..."
                } else if group.edits != group.saved_edits {
                    "Tab layout has unsaved changes."
                } else {
                    "Tab layout saved. Processes and output are not restored after restart."
                }),
        )
        .into_any_element()
    }
