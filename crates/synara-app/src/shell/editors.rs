    ...
    pub(super) fn editor_tools(&self, cx: &mut Context<Self>) -> gpui::AnyElement {
        ...
        div().flex().flex_col().gap_2().flex_shrink_0()
            .child(div().flex().items_center().flex_wrap().gap_1()
                ...
                .child(ui::action("editor-comment", "Comment on lines", Some(Glyph::Chat), false,
                    cx.listener(|this, _: &(), window, cx| this.open_file_comment(window, cx))).relative().child(ui::layout_probe("editor-comment")))
                .child(self.editor_actions(cx)))
            .children(self.file_comments.open().then(|| self.file_comment_panel(cx)))
