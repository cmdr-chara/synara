...
pub(super) enum Kind { Context, Added, Removed, Hunk, Meta }
#[derive(Debug)]
        ...
        for text in raw.lines() {
            ...
            let kind = if text.starts_with("diff --git ") {
                ...
                match text.as_bytes().first() {
                    Some(b'+') => { new = Some(*right); *right = right.saturating_add(1); result.added += 1; Kind::Added }
                    Some(b'-') => { old = Some(*left); *left = left.saturating_add(1); result.removed += 1; Kind::Removed }
                    Some(b' ') => { old = Some(*left); new = Some(*right); *left = left.saturating_add(1); *right = right.saturating_add(1); Kind::Context }
                    _ => Kind::Meta,
                }
            } else { Kind::Meta };
            if result.lines.len() < MAX_LINES {
                ...
                if clipped { text.push_str(" [line shortened]"); }
                result.limited |= clipped;
                result.lines.push(Line { old, new, text, kind });
...
pub(super) fn draft_with_diff(existing: &str, title: &str, raw: &str) -> Result<String, &'static str> {
    if raw.is_empty() { return Err("There is no diff to add. Untracked and binary files may have no text diff."); }
    if raw.len() > 128 * 1024 { return Err("This diff exceeds the 128 KiB draft-context limit. Select one file or copy the diff instead."); }
    ...
    if existing.len().saturating_add(addition.len()).saturating_add(2) > 1024 * 1024 {
        return Err("The chat draft would exceed 1 MiB. Your existing text is unchanged.");
    }
    Ok(if existing.is_empty() { addition } else { format!("{existing}\n\n{addition}") })
}
...
mod tests {
    ...
    fn headers_are_not_additions_and_hunk_line_numbers_are_exact() {
        let diff = Diff::parse("diff --git a/a b/a\n--- a/a\n+++ b/a\n@@ -10,2 +20,2 @@\n same\n-old\n+new\n\\ No newline at end of file\n");
    ...
    fn bounded_preview_retains_total_counts_and_clips_unicode_safely() {
        let raw = format!("@@ -0,0 +1,7000 @@\n{}+{}", "+line\n".repeat(7000), "界".repeat(3000));
